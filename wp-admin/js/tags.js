    ...
    if (r) {
      ...
      $.post(ajaxurl, data, function(r) {
        if ('1' == r) {
           ...
           */
          $(
            'select#parent option[value="' +
              data.match(/tag_ID=(\d+)/)[1] +
              '"]',
          ).remove();
          ...
          $('#ajax-response')
            .empty()
            .append('<div class="error"><p>' + tagsl10n.noPerm + '</p></div>');
          tr.children().css('backgroundColor', '');
        } else {
          $('#ajax-response')
            .empty()
            .append('<div class="error"><p>' + tagsl10n.broken + '</p></div>');
          tr.children().css('backgroundColor', '');
        }
      });
  ...
  $('#submit').click(function() {
    ...
    $.post(ajaxurl, $('#addtag').serialize(), function(r) {
      ...
      if (parent > 0 && $('#tag-' + parent).length > 0)
        ...
        $('.tags #tag-' + parent).after(
          res.responses[0].supplemental.noparents,
        );
      // As the parent exists, Insert the version with - - - prefixed
      else $('.tags').prepend(res.responses[0].supplemental.parents); // As the parent is not visible, Insert the version with Parent - Child - ThisTerm
