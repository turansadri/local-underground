...
(function(api, wp, $) {
        ...
        if ('page' === params.post_type) {
          ...
          api.control.each(function(control) {
            ...
            if ('dropdown-pages' === control.params.type) {
              select = control.container.find(
                'select[name^="_customize-dropdown-pages-"]',
              );
    ...
    /** @lends wp.customize.Menus.AvailableMenuItemsPanelView.prototype */ {
      ...
      initialize: function() {
        ...
        $('#customize-controls, .customize-section-back').on(
          ...
          function(e) {
            var isDeleteBtn = $(e.target).is('.item-delete, .item-delete *'),
              isAddNewBtn = $(e.target).is(
                '.add-new-menu-item, .add-new-menu-item *',
              );
            if (
              $('body').hasClass('adding-menu-items') &&
              !isDeleteBtn &&
              !isAddNewBtn
            ) {
              self.close();
        ...
        this.sectionContent.scroll(function() {
          ...
          if (
            !self.loading &&
            $(this).scrollTop() > (3 / 4) * totalHeight - visibleHeight
          ) {
      ...
      search: function(event) {
        var $searchSection = $('#available-menu-items-search'),
          $otherSections = $('#available-menu-items .accordion-section').not(
            $searchSection,
          );
      ...
      doSearch: function(page) {
        ...
        self.currentRequest = wp.ajax.post(
          'search-available-menu-items-customizer',
          params,
        );
        ...
        self.currentRequest.done(function(data) {
          ...
          if (items && page > 1) {
            wp.a11y.speak(
              api.Menus.data.l10n.itemsFoundMore.replace('%d', items.length),
            );
          } else if (items && page === 1) {
            wp.a11y.speak(
              api.Menus.data.l10n.itemsFound.replace('%d', items.length),
            );
          }
        });
        ...
        self.currentRequest.fail(function(data) {
          ...
          if (data.message) {
            $content
              .empty()
              .append($('<li class="nothing-found"></li>').text(data.message));
      ...
      loadItems: function(itemTypes, deprecated) {
        ...
        _.each(_itemTypes, function(itemType) {
          ...
          container = $(
            '#available-menu-items-' + itemType.type + '-' + itemType.object,
          );
        ...
        request.done(function(data) {
          ...
          _.each(data.items, function(typeItems, name) {
            ...
            typeInner = availableMenuItemContainers[name].find(
      ...
      submitNew: function(container) {
        ...
        promise.done(function(data) {
          ...
          itemElement = $(
            wp.template('available-menu-item')(availableItem.attributes),
          );
      ...
      open: function(menuControl) {
        ...
        _(this.currentMenuControl.getMenuItemControls()).each(function(
          control,
        ) {
      ...
      close: function(options) {
        ...
        $('#available-menu-items .menu-item-handle.item-added').removeClass(
          'item-added',
        );
    ...
    /** @lends wp.customize.Menus.MenuSection.prototype */ {
      ...
      ready: function() {
        ...
        handleFieldActiveToggle = function() {
          ...
          section.contentContainer.toggleClass(
            className,
            $(this).prop('checked'),
          );
        };
      ...
      refreshAssignedLocations: function() {
        ...
        _.each(section.navMenuLocationSettings, function(
          setting,
          themeLocation,
        ) {
      ...
      updateAssignedLocationsInSectionTitle: function(themeLocationSlugs) {
        ...
        _.each(themeLocationSlugs, function(themeLocationSlug) {
          ...
          locationName =
            api.Menus.data.locationSlugMappedToName[themeLocationSlug];
          $label.text(
            api.Menus.data.l10n.menuLocation.replace('%s', locationName),
          );
          $title.append($label);
        });
        ...
        if (expanded) {
          ...
          args.completeCallback = function() {
            if ('resolved' !== section.deferred.initSortables.state()) {
              ...
              api
                .control('nav_menu[' + String(section.params.menu_id) + ']')
                .reflowMenuItems();
            }
      ...
      highlightNewItemButton: function() {
        api.utils.highlightButton(
          this.contentContainer.find('.add-new-menu-item'),
          { delay: 2000 },
        );
      },
    },
  ...
  api.Menus.NewMenuSection = api.Section.extend(
    /** @lends wp.customize.Menus.NewMenuSection.prototype */ {
      ...
      attachEvents: function() {
        ...
        contentContainer.on('click', '#customize-new-menu-submit', function(
          event,
        ) {
          section.submit();
        ...
        function getNavMenuCount() {
          ...
          api.each(function(setting) {
            if (
              navMenuSettingPattern.test(setting.id) &&
              false !== setting.get()
            ) {
              count += 1;
            }
        ...
        function updateNoticeVisibility() {
          container
            .find('.add-new-menu-notice')
            .prop('hidden', getNavMenuCount() > 0);
        }
      ...
      submit: function() {
        ...
        contentContainer
          .find('.assigned-menu-location input[type=checkbox]')
          .each(function() {
            var checkbox = $(this),
    ...
    /** @lends wp.customize.Menus.MenuLocationControl.prototype */ {
      ...
      ready: function() {
        ...
        control.setting.bind('change', function() {
          ...
          control.container
            .find('.create-menu')
            .toggleClass('hidden', menuIsSelected);
          control.container
            .find('.edit-menu')
            .toggleClass('hidden', !menuIsSelected);
        });
    ...
    /** @lends wp.customize.Menus.MenuItemControl.prototype */ {
      ...
      embed: function() {
        ...
        if (
          ...
          api.settings.autofocus.control === control.id
        ) {
          control.actuallyEmbed();
        }
      },
      ...
      _setupReorderUI: function() {
        ...
        $reorderNav
          .find(
            '.menus-move-up, .menus-move-down, .menus-move-left, .menus-move-right',
          )
          .on('click', function() {
            var moveBtn = $(this);
            moveBtn.focus();
      ...
      _setupUpdateUI: function() {
        ...
        control.elements = {};
        control.elements.url = new api.Element(
          control.container.find('.edit-menu-item-url'),
        );
        ...
        control.elements.xfn = new api.Element(
          control.container.find('.edit-menu-item-xfn'),
        );
        ...
        control.setting.bind(function(to, from) {
          ...
          if (false === to) {
            menuControl = api.control(
              'nav_menu[' + String(from.nav_menu_term_id) + ']',
            );
            control.container.remove();

            _.each(menuControl.getMenuItemControls(), function(otherControl) {
              if (
                from.menu_item_parent ===
            ...
            _.each(followingSiblingItemControls, function(
              followingSiblingItemControl,
            ) {
            ...
            control.container
              ...
              .val(to.menu_item_parent);

            // Handle UI updates when the position or depth (parent) change.
            if (
              to.position !== from.position ||
              to.menu_item_parent !== from.menu_item_parent
            ) {
      ...
      _setupRemoveUI: function() {
        ...
        $removeBtn.on('click', function() {
          ...
          $next = control.container
            .nextAll('.customize-control-nav_menu_item:visible')
            .first();
          $prev = control.container
            .prevAll('.customize-control-nav_menu_item:visible')
            .first();
      ...
      _setupTitleUI: function() {
        ...
        control.setting.bind(function(item) {
          ...
          titleText =
            trimmedTitle || item.original_title || api.Menus.data.l10n.untitled;

          if (item._invalid) {
            titleText = api.Menus.data.l10n.invalidTitleTpl.replace(
              '%s',
              titleText,
            );
          }
      ...
      getDepth: function() {
        ...
        while (setting && setting.menu_item_parent) {
          depth += 1;
          control = api.control(
            'nav_menu_item[' + setting.menu_item_parent + ']',
          );
          if (!control) {
            break;
          }
      ...
      onChangeExpanded: function(showOrHide, params) {
        ...
        if (showOrHide) {
          ...
          api.control.each(function(otherControl) {
            if (
              self.params.type === otherControl.params.type &&
              self !== otherControl
            ) {
      ...
      _changePosition: function(offset) {
        ...
        _(control.getMenuControl().getMenuItemControls()).each(function(
          otherControl,
        ) {
          if (
            otherControl.setting().menu_item_parent ===
      ...
      _changeDepth: function(offset) {
        ...
        if (-1 === offset) {
          ...
          parentControl = api.control(
            'nav_menu_item[' + settingValue.menu_item_parent + ']',
          );
          ...
          _(control.getMenuControl().getMenuItemControls()).each(function(
            ...
            isControlToBeShifted =
              ...
              otherControl.setting().position >
                parentControl.setting().position;
    ...
    /** @lends wp.customize.Menus.MenuNameControl.prototype */ {
      ready: function() {
        ...
        if (control.setting) {
          ...
          control.nameElement = new api.Element(
            control.container.find('.menu-name-field'),
          );
        ...
        control.container.find('.assigned-menu-location').each(function() {
          var container = $(this),
            ...
            updateSetting = isNewMenu
              ? _.noop
              : function(checked) {
                  navMenuLocationSetting.set(
                    checked ? control.params.menu_id : 0,
                  );
                },
          ...
          updateCheckbox(
            navMenuLocationSetting.get() === control.params.menu_id,
          );
      ...
      setSelections: function(selections) {
        this.container.find('.menu-location').each(function(i, checkboxNode) {
          ...
          checkboxNode.checked =
            locationId in selections ? selections[locationId] : false;
        });
      },
    },
  );
    ...
    /** @lends wp.customize.Menus.MenuControl.prototype */ {
      ...
      ready: function() {
        ...
        if (menu) {
          ...
          api.control.each(function(widgetControl) {
            ...
            if (
              0 === select.find('option[value=' + String(menuId) + ']').length
            ) {
      ...
      _setupSortable: function(menuList) {
        ...
        menuList.on('sortstop', function() {
          setTimeout(function() {
            // Next tick.
            var menuItemContainerIds = control.$sectionContent.sortable(
                'toArray',
              ),
              menuItemControls = [],
              position = 0,
            ...
            _.each(menuItemContainerIds, function(menuItemContainerId) {
              ...
              menuItemControl = api.control(
                'nav_menu_item[' + String(menuItemId) + ']',
              );
              if (menuItemControl) {
      ...
      _setupTitle: function() {
        ...
        control.setting.bind(function(menu) {
          ...
          var section = api.section(control.section()),
            menuId = control.params.menu_id,
            controlTitle = section.headContainer.find(
              '.accordion-section-title',
            ),
            sectionTitle = section.contentContainer.find(
          ...
          section.contentContainer
            .find('.customize-control-checkbox input')
            .each(function() {
              if ($(this).prop('checked')) {
                $(
                  '.current-menu-location-name-' + $(this).data('location-id'),
                ).text(name);
              }
            });
        });
      ...
      reflowMenuItems: function() {
        ...
        reflowRecursively = function(context) {
          ...
          if (currentMenuItemControls.length) {
            ...
            currentMenuItemControls[0].container
              ...
              .toggleClass(
                'move-down-disabled',
                1 === currentMenuItemControls.length,
              );
            currentMenuItemControls[
              currentMenuItemControls.length - 1
            ].container
        ...
        menuControl.container
          .find('.reorder-toggle')
          .toggle(menuItemControls.length > 1);
      },
      ...
      updateInvitationVisibility: function(optionalMenuItemControls) {
        var menuItemControls =
          optionalMenuItemControls || this.getMenuItemControls();

        this.container
          .find('.new-menu-item-invitation')
          .toggle(menuItemControls.length === 0);
      },
    },
  );

  api.Menus.NewMenuControl = api.Control.extend(
    /** @lends wp.customize.Menus.NewMenuControl.prototype */ {
      /**
      ...
      initialize: function() {
        if ('undefined' !== typeof console && console.warn) {
          console.warn(
            '[DEPRECATED] wp.customize.NewMenuControl will be removed. Please use wp.customize.Menus.createNavMenu() instead.',
          );
        }
  ...
  api.bind('ready', function() {
    ...
    api.previewer.bind(
      'focus-nav-menu-item-control',
      api.Menus.focusMenuItemControl,
    );
  });
  ...
  api.Menus.applySavedData = function(data) {
      ...
      if ('inserted' === update.status) {
        ...
        api.control.each(function(setting) {
          ...
          oldMenuOption = select.find(
            'option[value=' + String(update.previous_term_id) + ']',
          );
          newMenuOption = select.find(
            'option[value=' + String(update.term_id) + ']',
          );
    ...
    _(data.nav_menu_item_updates).each(function(update) {
      ...
      if ('inserted' === update.status) {
        ...
        oldCustomizeId =
          'nav_menu_item[' + String(update.previous_post_id) + ']';
        if (!api.has(oldCustomizeId)) {
          throw new Error('Expected setting to exist: ' + oldCustomizeId);
        }
        oldSetting = api(oldCustomizeId);
        ...
        if (insertedMenuIdMapping[settingValue.nav_menu_term_id]) {
          settingValue.nav_menu_term_id =
            insertedMenuIdMapping[settingValue.nav_menu_term_id];
        }
     ...
     */
    _.each(data.widget_nav_menu_updates, function(
      widgetSettingValue,
      widgetSettingId,
    ) {
  ...
  function displayNavMenuName(name) {
    ...
    name = $('<div>')
      .text(name)
      .html(); // Emulate esc_html() which is used in wp-admin/nav-menus.php.
    name = $.trim(name);
