  ...
  $(function() {
    ...
    function textEditorResize() {
      ...
      if (length < oldTextLength) {
        ...
        parseInt(textEditor.style.height, 10) < textEditor.scrollHeight
      ) {
    ...
    $document.on('tinymce-editor-init.editor-expand', function(event, editor) {
      ...
      function mceScroll(key) {
        ...
        cursorTop =
          offset.top + editor.iframeElement.getBoundingClientRect().top;
        ...
        editorBottom =
          ...
          (advanced ? heights.bottomHeight + heights.statusBarHeight : 0);
        ...
        if (
          cursorTop < editorTop &&
          (key === VK.UP || key === VK.LEFT || key === VK.BACKSPACE)
        ) {
          window.scrollTo(
            window.pageXOffset,
            cursorTop + window.pageYOffset - editorTop,
          );
          ...
          window.scrollTo(
            window.pageXOffset,
            cursorBottom + window.pageYOffset - editorBottom,
          );
        }
      }
      ...
      mceBind = function() {
        ...
        $window
          .off('scroll.mce-float-panels')
          .on('scroll.mce-float-panels', hideFloatPanels);
      };
    ...
    function adjust(event) {
       ...
       */
      canPin = visual
        ? autoresizeMinHeight + topHeight
        : autoresizeMinHeight + 20; // 20px from textarea padding
      ...
      if (!canPin) {
        ...
        if (
          ...
          if (
            windowPos <=
            topPos - heights.toolsHeight - heights.adminBarHeight
          ) {
            fixedTop = false;
      ...
      if (
        ...
        if (
          heights.sideSortablesHeight + pinnedToolsTop + sidebarBottom >
            heights.windowHeight ||
          fixedSideTop ||
          fixedSideBottom
        ) {
          ...
          if (windowPos + pinnedToolsTop <= postBodyTop) {
            ...
            if (windowPos > lastScrollPosition) {
              if (fixedSideTop) {
                ...
                fixedSideTop = false;
                sidebarTop =
                  $sideSortables.offset().top - heights.adminBarHeight;
                footerTop = $footer.offset().top;

                // Don't get over the footer.
                if (
                  footerTop <
                  sidebarTop + heights.sideSortablesHeight + sidebarBottom
                ) {
                ...
                !fixedSideBottom &&
                heights.sideSortablesHeight +
                  $sideSortables.offset().top +
                  sidebarBottom <
                  windowPos + heights.windowHeight
              ) {
                // Pin the bottom.
        ...
        .on(
          ...
          function() {
            if (
              !fixedSideTop &&
              !fixedSideBottom &&
              window.pageYOffset > pinnedToolsTop
            ) {
              fixedSideBottom = true;
              window.scrollBy(0, -1);
              adjust();
  ...
  $(function() {
    ...
    function fadeOut(event) {
      ...
      if (
        ...
        (key === 87 &&
          event.altKey &&
          ((!isMac && event.shiftKey) || (isMac && event.ctrlKey)))
      ) {
        fadeIn(event);
        return;
      }
    ...
    function fadeIn(event) {
      if (faded) {
        ...
        $overlay.off(
          'mouseenter.focus mouseleave.focus mousemove.focus touchstart.focus',
        );
        ...
        if ('undefined' === typeof event) {
          $editor.on('mouseenter.focus', function() {
            if (
              $.contains($editor.get(0), document.activeElement) ||
              editorHasFocus
            ) {
              fadeOut();
            }
    ...
    function maybeFadeIn() {
      setTimeout(function() {
        var position = document.activeElement.compareDocumentPosition(
          $editor.get(0),
        );
    ...
    $document
      .on('editor-expand-on.focus', activate)
      .on('editor-expand-off.focus', deactivate);
