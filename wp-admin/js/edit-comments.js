...
(function($) {
  ...
  updateDashboardText = function(response) {
    ...
    $('.comment-count a', '#dashboard_right_now').text(
      response.i18n_comments_text,
    );
  };
  ...
  updateHtmlTitle = function(diff) {
    ...
    titleRegEx =
      titleRegEx ||
      new RegExp(
        adminCommentsL10n.docTitleCommentsCount.replace(
          '%s',
    ...
    if (titleCount >= 1) {
      ...
      if (regExMatch) {
        newTitle = newTitle.replace(
          regExMatch[0],
          adminCommentsL10n.docTitleComments,
        );
      }
    }
  ...
  updatePending = function(diff, commentPostId) {
    ...
    $('span.pending-count').each(function() {
      var a = $(this),
        n = getCount(a) + diff;
      if (n < 1) n = 0;
      a.closest('.awaiting-mod')[0 === n ? 'addClass' : 'removeClass'](
        'count-0',
      );
      updateCount(a, n);
    });
  ...
  window.setCommentsList = function() {
    ...
    delBefore = function(settings, list) {
      ...
      if (action) {
        ...
        a.attr(
          'data-wp-lists',
          'delete:the-comment-list:comment-' + id + '::un' + action + '=1',
        );
    ...
    delAfter = function(r, settings) {
      var total_items_i18n,
        ...
        commentStatus =
          true === settings.parsed ? '' : response.supplemental.status,
        commentPostId =
          true === settings.parsed ? '' : response.supplemental.postId,
         ...
         */
        approved =
          commentRow.hasClass('approved') && !commentRow.hasClass('unapproved'),
      ...
      if (
        ('trash' === settings.data.comment_status &&
          !getCount($('span.trash-count'))) ||
        ('spam' === settings.data.comment_status &&
          !getCount($('span.spam-count')))
      ) {
        $('#delete_all').hide();
      }
    ...
    window.theList = $('#the-comment-list')
      ...
      .bind('wpListDelEnd', function(e, s) {
        ...
        if (
          wpListsData.indexOf(':trash=1') != -1 ||
          wpListsData.indexOf(':spam=1') != -1
        )
  ...
  window.commentReply = {
    ...
    init: function() {
      ...
      $(
        'input#author-name, input#author-email, input#author-url',
        row,
      ).keypress(function(e) {
        if (e.which == 13) {
          commentReply.send();
      ...
      this.comments_listing =
        $('#comments-form > input[name="comment_status"]').val() || '';
    },
    ...
    toggle: function(el) {
      if (
        ...
        ($('#replyrow')
          .parent()
          .is('#com-reply') ||
          window.confirm(adminCommentsL10n.warnQuickEdit))
      ) {
        $(el)
          .find('a.vim-q')
          .click();
    ...
    open: function(comment_id, post_id, action) {
      ...
      if (action == 'edit') {
        ...
        $(
          '#replyhead, #replybtn, #edithead, #editlegend, #savebtn',
          editRow,
        ).hide();
        ...
        replyButton = $('#replybtn', editRow);
        $(
          '#edithead, #editlegend, #savebtn, #addhead, #addbtn',
          editRow,
        ).hide();
