...
(function($) {
        ...
        this.load(diffs)
          ...
          .fail(function() {
            if (1 === num) {
              ...
              self
                ._loadAll(allRevisionIds, centerId, Math.ceil(num / 2))
                .done(function() {
                  deferred.resolve();
                });
            }
  ...
  revisions.view.Checkbox = wp.Backbone.View.extend({
    ...
    updateCompareTwoMode: function() {
      this.$('.compare-two-revisions').prop(
        'checked',
        this.model.get('compareTwoMode'),
      );
    },
    ...
    compareTwoToggle: function() {
      ...
      this.model.set({
        compareTwoMode: $('.compare-two-revisions').prop('checked'),
      });
    },
  });
      ...
      $(window).on('mousemove.wp.revisions', { view: this }, function(e) {
        ...
        if (view.model.get('compareTwoMode')) {
          ...
          if (handle.is(handles.first())) {
            ...
            leftDragBoundary =
              handles.first().offset().left + handles.first().width();
    ...
    slide: function(event, ui) {
      ...
      if (this.model.get('compareTwoMode')) {
        ...
        if (this.getPosition(ui.value) > 0) {
          attributes.from = this.model.revisions.at(
            this.getPosition(ui.value) - 1,
          );
        } else {
  ...
  revisions.init = function() {
    ...
    state = new revisions.model.FrameState(
      ...
      {
        revisions: new revisions.model.Revisions(
          revisions.settings.revisionData,
        ),
      },
    );
