...
wp.mediaWidgets = (function($) {
  ...
  component.PersistentDisplaySettingsLibrary = wp.media.controller.Library.extend(
    /** @lends wp.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */ {
       ...
       * @returns {void}
       */
      handleDisplaySettingChange: function handleDisplaySettingChange(
        displaySettings,
      ) {
      ...
      display: function getDisplaySettingsModel(model) {
        ...
        display = wp.media.controller.Library.prototype.display.call(
          this,
          model,
        );
        ...
        if ('image' === this.controller.options.mimeType) {
          ...
          Constructor = wp.media.view.EmbedLink.extend(
            /** @lends wp.mediaWidgets~Constructor.prototype */ {
               ...
               * @returns {void}
               */
              setAddToWidgetButtonDisabled: function setAddToWidgetButtonDisabled(
                disabled,
              ) {
              ...
              setErrorNotice: function setErrorNotice(notice) {
                ...
                noticeContainer = embedLinkView.views.parent.$el.find(
                  '> .notice:first-child',
                );
                if (!notice) {
              ...
              updateoEmbed: function() {
                ...
                if (!url.match(/^(http|https):\/\/.+\//)) {
                  embedLinkView.controller.$el
                    .find('#embed-url-field')
                    .addClass('invalid');
                  embedLinkView.setAddToWidgetButtonDisabled(true);
                }

                wp.media.view.EmbedLink.prototype.updateoEmbed.call(
                  embedLinkView,
                );
              },
              ...
              fetch: function() {
                ...
                if (
                  embedLinkView.dfd &&
                  'pending' === embedLinkView.dfd.state()
                ) {
                ...
                if (youTubeEmbedMatch) {
                  url =
                    'https://www.youtube.com/watch?v=' + youTubeEmbedMatch[1];
                ...
                embedLinkView.dfd.done(function(response) {
                  if (
                    embedLinkView.controller.options.mimeType !== response.type
                  ) {
                    embedLinkView.renderFail();
                    return;
                  }
                ...
                embedLinkView.dfd.fail(
                  _.bind(embedLinkView.renderFail, embedLinkView),
                );
              },
              ...
              renderFail: function renderFail() {
                ...
                embedLinkView.controller.$el
                  ...
                  .addClass('invalid');
                embedLinkView.setErrorNotice(
                  embedLinkView.controller.options.invalidEmbedTypeError ||
                    'ERROR',
                );
                embedLinkView.setAddToWidgetButtonDisabled(true);
              },
    ...
    /** @lends wp.mediaWidgets.MediaWidgetControl.prototype */ {
      ...
      initialize: function initialize(options) {
        ...
        control.listenTo(
          control.model,
          'change',
          control.syncModelToPreviewProps,
        );
      ...
      updateSelectedAttachment: function updateSelectedAttachment() {
        ...
        if (0 === control.model.get('attachment_id')) {
          ...
          control.model.get('attachment_id') !==
          control.selectedAttachment.get('id')
        ) {
      ...
      syncModelToPreviewProps: function syncModelToPreviewProps() {
        var control = this;
        control.previewTemplateProps.set(
          control.mapModelToPreviewTemplateProps(),
        );
      },
      ...
      syncModelToInputs: function syncModelToInputs() {
        ...
        control.syncContainer
          ...
          .each(function() {
            ...
            if (
              'array' === control.model.schema[propertyName].type &&
              _.isArray(value)
            ) {
              value = value.join(',');
      ...
      template: function template() {
        ...
        if (!$('#tmpl-widget-media-' + control.id_base + '-control').length) {
          throw new Error(
            'Missing widget control template for ' + control.id_base,
          );
        }
      ...
      isSelected: function isSelected() {
        ...
        return Boolean(
          control.model.get('attachment_id') || control.model.get('url'),
        );
      },
      ...
      selectMedia: function selectMedia() {
        ...
        selection = new wp.media.model.Selection(selectionModels, {
          multiple: false,
        });

        mediaFrameProps = control.mapModelToMediaFrameProps(
        ...
        mediaFrame = new component.MediaFrameSelect({
          ...
          state:
            control.isSelected() && 0 === control.model.get('attachment_id')
              ? 'embed'
              : 'insert',
         ...
         */
        mediaFrame.$el
          .find('.media-frame-menu .media-menu-item.active')
          .focus();
      },
       ...
       */
      getModelPropsFromMediaFrame: function getModelPropsFromMediaFrame(
        mediaFrame,
      ) {
        var control = this,
          state,
        ...
        if ('insert' === state.get('id')) {
          ...
          if (
            ...
            mediaFrameProps.url =
              mediaFrameProps.sizes[mediaFrameProps.size].url;
          }
        ...
        _.each(wp.media.view.settings.embedExts, function(ext) {
          if (
            ext in control.model.schema &&
            modelProps.url !== modelProps[ext]
          ) {
            modelProps[ext] = '';
          }
        });
      ...
      mapMediaToModelProps: function mapMediaToModelProps(mediaFrameProps) {
        ...
        _.each(control.model.schema, function(fieldSchema, modelProp) {
          ...
          mediaFramePropToModelPropMap[
            fieldSchema.media_prop || modelProp
          ] = modelProp;
        });
        ...
        if ('post' === mediaFrameProps.link) {
          modelProps.link_url =
            mediaFrameProps.postUrl || mediaFrameProps.linkUrl;
      ...
      mapModelToPreviewTemplateProps: function mapModelToPreviewTemplateProps() {
        ...
        _.each(control.model.schema, function(value, prop) {
          if (
            !value.hasOwnProperty('should_preview_update') ||
            value.should_preview_update
          ) {
      ...
      set: function set(key, val, options) {
        ...
        _.each(attrs, function(value, name) {
          ...
          if ('array' === type) {
            ...
            if (
              model.schema[name].items &&
              'integer' === model.schema[name].items.type
            ) {
  ...
  component.handleWidgetAdded = function handleWidgetAdded(
    ...
    ModelConstructor =
      component.modelConstructors[idBase] || component.MediaWidgetModel;
  ...
  component.setupAccessibleMode = function setupAccessibleMode() {
    ...
    component.widgetControls[
      widgetControl.model.get('widget_id')
    ] = widgetControl;
  ...
  component.handleWidgetUpdated = function handleWidgetUpdated(
    ...
    widgetControl.stopListening(
      widgetControl.model,
      'change',
      widgetControl.syncModelToInputs,
    );
    widgetControl.model.set(attributes);
    widgetControl.listenTo(
      widgetControl.model,
      'change',
      widgetControl.syncModelToInputs,
    );
  ...
  component.init = function init() {
    ...
    $(function initializeExistingWidgetContainers() {
      ...
      widgetContainers = $('.widgets-holder-wrap:not(#available-widgets)').find(
        'div.widget',
      );
