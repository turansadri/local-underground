...
(function($, wp, settings) {
  ...
  wp.updates.refreshCount = function() {
    var $adminBarUpdates = $('#wp-admin-bar-updates'),
      $dashboardNavMenuUpdateCount = $(
        'a[href="update-core.php"] .update-plugins',
      ),
    ...
    if (settings.totals.counts.total > 0) {
      $dashboardNavMenuUpdateCount
        .find('.update-count')
        .text(settings.totals.counts.total);
    } else {
    ...
    if (settings.totals.counts.total > 0) {
      $pluginsNavMenuUpdateCount
        .find('.plugin-count')
        .text(settings.totals.counts.plugins);
    } else {
    ...
    if (settings.totals.counts.total > 0) {
      $appearanceNavMenuUpdateCount
        .find('.theme-count')
        .text(settings.totals.counts.themes);
    } else {
  ...
  wp.updates.decrementCount = function(type) {
    ...
    if ('plugin' === type) {
      settings.totals.counts.plugins = Math.max(
        --settings.totals.counts.plugins,
        0,
      );
    } else if ('theme' === type) {
      settings.totals.counts.themes = Math.max(
  ...
  wp.updates.updatePlugin = function(args) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      $message = $updateRow
        ...
        .find('p');
      message = wp.updates.l10n.pluginUpdatingLabel.replace(
        '%s',
    ...
    $message.attr('aria-label', message).text(wp.updates.l10n.updating);
  ...
  wp.updates.updatePluginSuccess = function(response) {
    ...
    $updateMessage
      .attr(
        'aria-label',
        wp.updates.l10n.pluginUpdatedLabel.replace('%s', response.pluginName),
      )
  ...
  wp.updates.updatePluginError = function(response) {
    ...
    errorMessage = wp.updates.l10n.updateFailed.replace(
      '%s',
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      if (response.plugin) {
        $message = $('tr[data-plugin="' + response.plugin + '"]').find(
          '.update-message',
        );
      } else {
        $message = $('tr[data-slug="' + response.slug + '"]').find(
          '.update-message',
        );
      }
      $message
      ...
      if (response.pluginName) {
        $message
          ...
          .attr(
            'aria-label',
            wp.updates.l10n.pluginUpdateFailedLabel.replace(
              '%s',
              response.pluginName,
            ),
          );
  ...
  wp.updates.installPlugin = function(args) {
    ...
    $message
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.pluginInstallingLabel.replace(
          '%s',
          $message.data('name'),
        ),
      )
    ...
    if (response.activateUrl) {
      setTimeout(function() {
        ...
        $message
          ...
          .attr(
            'aria-label',
            wp.updates.l10n.activatePluginLabel.replace(
              '%s',
              response.pluginName,
            ),
          )
  ...
  wp.updates.installPluginError = function(response) {
    ...
    errorMessage = wp.updates.l10n.installFailed.replace(
      '%s',
      response.errorMessage,
    );
    ...
    $button
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.pluginInstallFailedLabel.replace(
          '%s',
          $button.data('name'),
        ),
      )
  ...
  wp.updates.installImporterSuccess = function(response) {
    wp.updates.addAdminNotice({
      ...
      message: wp.updates.l10n.importerInstalledMsg.replace(
        '%s',
    ...
    $('[data-slug="' + response.slug + '"]')
      ...
      .attr({
        ...
        'aria-label': wp.updates.l10n.activateImporterLabel.replace(
          '%s',
          response.pluginName,
        ),
      })
  ...
  wp.updates.installImporterError = function(response) {
    var errorMessage = wp.updates.l10n.installFailed.replace(
        '%s',
        response.errorMessage,
      ),
    ...
    $installLink
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.pluginInstallNowLabel.replace('%s', pluginName),
      );
  ...
  wp.updates.deletePlugin = function(args) {
    var $link = $('[data-plugin="' + args.plugin + '"]').find(
    ...
    if ($link.html() !== wp.updates.l10n.deleting) {
      $link.data('originaltext', $link.html()).text(wp.updates.l10n.deleting);
    }
    ...
    $('[data-plugin="' + response.plugin + '"]')
      ...
      .fadeOut(350, function() {
        ...
        if (-1 !== _.indexOf(plugins.recently_activated, response.plugin)) {
          plugins.recently_activated = _.without(
            plugins.recently_activated,
            response.plugin,
          );
  ...
  wp.updates.deletePluginError = function(response) {
    ...
    if (response.plugin) {
      ...
      $pluginUpdateRow = $plugin.siblings(
        '[data-plugin="' + response.plugin + '"]',
      );
    } else {
    ...
    if (!$pluginUpdateRow.length) {
      $plugin.addClass('update').after(
        pluginUpdateRow({
          ...
          colspan: $('#bulk-action-form').find(
            'thead th:not(.hidden), thead td',
          ).length,
  ...
  wp.updates.updateTheme = function(args) {
    ...
    if ('themes-network' === pagenow) {
      ...
      $notice = $('[data-slug="' + args.slug + '"].notice').removeClass(
        'notice-large',
      );
      ...
      $notice = $notice.add(
        $('#customize-control-installed_theme_' + args.slug).find(
          '.update-message',
        ),
      );
      ...
      $notice = $notice.add(
        $('[data-slug="' + args.slug + '"]').find('.update-message'),
      );
      $notice = $notice.addClass('updating-message').find('p');
    }
  ...
  wp.updates.updateThemeSuccess = function(response) {
    ...
    if (isModalOpen && 'customize' !== pagenow) {
      $('.theme-info .theme-author').after(
        wp.updates.adminNotice(updatedMessage),
      );
    }
  };
  ...
  wp.updates.installTheme = function(args) {
    ...
    $message
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.themeInstallingLabel.replace(
          '%s',
          $message.data('name'),
        ),
      );
  ...
  wp.updates.installThemeSuccess = function(response) {
    ...
    $message = $card
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.themeInstalledLabel.replace('%s', response.themeName),
      )
    ...
    setTimeout(function() {
      if (response.activateUrl) {
        ...
        $message
          ...
          .attr(
            'aria-label',
            wp.updates.l10n.activateThemeLabel.replace(
              '%s',
              response.themeName,
            ),
          )
  ...
  wp.updates.installThemeError = function(response) {
    ...
    $button
      ...
      .attr(
        'aria-label',
        wp.updates.l10n.themeInstallFailedLabel.replace(
          '%s',
          $button.data('name'),
        ),
      )
  ...
  wp.updates.deleteTheme = function(args) {
    ...
    if ('themes' === pagenow) {
      ...
      $button = $('[data-slug="' + args.slug + '"]').find(
        '.row-actions a.delete',
      );
    }
  ...
  wp.updates.deleteThemeError = function(response) {
    var $themeRow = $('tr.inactive[data-slug="' + response.slug + '"]'),
      ...
      errorMessage = wp.updates.l10n.deleteFailed.replace(
        '%s',
        response.errorMessage,
      ),
      $message = wp.updates.adminNotice({
  ...
  wp.updates.showErrorInCredentialsForm = function(message) {
    ...
    $filesystemForm
      ...
      .after(
        '<div class="notice notice-alt notice-error"><p>' +
          message +
          '</p></div>',
      );
  };
  ...
  wp.updates.isValidResponse = function(response, action) {
    ...
    if (_.isString(response) && '-1' === response) {
      ...
      'undefined' !== typeof response.readyState &&
      0 === response.readyState
    ) {
      error = wp.updates.l10n.connectionError;
    } else if (
      _.isString(response.responseText) &&
  ...
  $(function() {
    ...
    $document.on('credential-modal-cancel', function(event, job) {
      ...
      if ('import' === pagenow) {
        ...
        if ('update-plugin' === job.action) {
          $message = $('tr[data-plugin="' + job.data.plugin + '"]').find(
            '.update-message',
          );
        } else if ('delete-plugin' === job.action) {
          $message = $('[data-plugin="' + job.data.plugin + '"]').find(
            '.row-actions a.delete',
          );
        }
      } else if ('themes' === pagenow || 'themes-network' === pagenow) {
      ...
      if ($message && $message.hasClass('updating-message')) {
        ...
        if (
          ...
          if ('update-plugin' === job.action) {
            $message.attr(
              'aria-label',
              wp.updates.l10n.pluginUpdateNowLabel.replace(
                '%s',
                $message.data('name'),
              ),
            );
            ...
            $message.attr(
              'aria-label',
              wp.updates.l10n.pluginInstallNowLabel.replace(
                '%s',
                $message.data('name'),
              ),
            );
    ...
    $bulkActionForm.on('click', '[data-plugin] .update-link', function(event) {
      ...
      if (
        ...
        $message.hasClass('button-disabled')
      ) {
        return;
      }
      ...
      wp.updates.$elToReturnFocusToFromCredentialsModal = $pluginRow.find(
        '.check-column input',
      );
    ...
    $bulkActionForm.on('click', '[data-plugin] a.delete', function(event) {
      ...
      if (
        !window.confirm(
          wp.updates.l10n.aysDeleteUninstall.replace(
            '%s',
     ...
     * @param {Event} event Event interface.
     */
    $document.on('click', '.themes-php.network-admin .update-link', function(
      event,
    ) {
      var $message = $(event.target),
      ...
      wp.updates.$elToReturnFocusToFromCredentialsModal = $themeRow.find(
        '.check-column input',
      );
     ...
     * @param {Event} event Event interface.
     */
    $document.on('click', '.themes-php.network-admin a.delete', function(
      event,
    ) {
      ...
      if (
        !window.confirm(
          wp.updates.l10n.aysDelete.replace(
            '%s',
    ...
    $bulkActionForm.on(
      ...
      function(event) {
        var bulkAction = $(event.target)
            .siblings('select')
            .val(),
          itemsSelected = $bulkActionForm.find(
        ...
        switch (bulkAction) {
          ...
          case 'delete-selected':
            if (
              !window.confirm(
                'plugin' === type
                  ? wp.updates.l10n.aysBulkDelete
                  : wp.updates.l10n.aysBulkDeleteThemes,
              )
            ) {
              event.preventDefault();
              return;
        ...
        $bulkActionForm
          .find('.manage-column [type="checkbox"]')
          .prop('checked', false);
        ...
        wp.updates.searchRequest = wp.ajax
          ...
          .done(function(response) {
            ...
            if (0 === response.count) {
              ...
              wp.a11y.speak(
                wp.updates.l10n.pluginsFound.replace('%d', response.count),
              );
            }
          });
      }, 500),
    ...
    $pluginSearch.on(
      ...
      _.debounce(function(event) {
        ...
        wp.updates.searchRequest = wp.ajax
          .post('search-plugins', data)
          .done(function(response) {
            // Can we just ditch this whole subtitle business?
            var $subTitle = $('<span />')
                .addClass('subtitle')
                .html(
                  wp.updates.l10n.searchResults.replace('%s', _.escape(data.s)),
                ),
