  ...
  api = window.wpNavMenu = {
    ...
    jQueryExtensions: function() {
      ...
      $.fn.extend({
        menuItemDepth: function() {
          var margin = api.isRTL
            ? this.eq(0).css('margin-right')
            : this.eq(0).css('margin-left');
          return api.pxToDepth(
            margin && -1 != margin.indexOf('px') ? margin.slice(0, -2) : 0,
          );
        },
        ...
        shiftDepthClass: function(change) {
          return this.each(function() {
            ...
            t.removeClass('menu-item-depth-' + depth).addClass(
              'menu-item-depth-' + newDepth,
            );
        ...
        addSelectedToMenu: function(processMethod) {
          ...
          return this.each(function() {
            ...
            $(checkboxes).each(function() {
              ...
              menuItems[listItemDBID] = t
                .closest('li')
                .getItemData('add-menu-item', listItemDBID);
            });
    ...
    refreshAdvancedAccessibilityOfItem: function(itemToRefresh) {
      ...
      var thisLink,
        ...
        hasSameDepthSibling = menuItem.nextAll('.menu-item-depth-' + depth)
          .length;
      ...
      if (0 !== position) {
        if (
          ...
          thisLink
            .attr(
              'aria-label',
              menus.moveUnder.replace('%s', prevItemNameRight),
            )
            .text(thisLinkText)
            .css('display', 'inline');
        }
      ...
      if (isPrimaryMenuItem) {
        ...
        (parentItem = menuItem
          ...
          (subItems = $(
            '.menu-item .menu-item-data-parent-id[value="' +
              parentItemId +
              '"]',
          )),
          (itemPosition =
    ...
    initPreviewing: function() {
      ...
      $('#menu-to-edit').on('change input', '.edit-menu-item-title', function(
        e,
      ) {
    ...
    initSortables: function() {
      ...
      api.menuList.sortable({
        ...
        start: function(e, ui) {
          ...
          parent =
            ui.item.next()[0] == ui.placeholder[0] ? ui.item.next() : ui.item;
          ...
          height +=
            height > 0 ? ui.placeholder.css('margin-top').slice(0, -2) * 1 : 0;
        ...
        sort: function(e, ui) {
          ...
          if (
            depth > maxDepth ||
            offset.top < prevBottom - api.options.targetTolerance
          ) {
            depth = maxDepth;
      ...
      function updateMenuMaxDepth(depthChange) {
        ...
        if (depthChange === 0) {
          ...
          while (
            !$('.menu-item-depth-' + newDepth, api.menuList).length &&
            newDepth > 0
          )
            newDepth--;
        }
        ...
        body
          .removeClass('menu-max-depth-' + menuMaxDepth)
          .addClass('menu-max-depth-' + newDepth);
    ...
    attachThemeLocationsListeners: function() {
      ...
      params['menu-settings-column-nonce'] = $(
    ...
    attachUnsavedChangesListener: function() {
      ...
      if (
        0 !== $('#menu-to-edit').length ||
        0 !== $('.menu-location-menus select').length
      ) {
        window.onbeforeunload = function() {
          if (api.menusChanged) return navMenuL10n.saveAlert;
        };
      } else {
        ...
        if (target.hasClass('nav-tab-link')) {
          ...
          if (selectAreaMatch && selectAreaMatch[1]) {
            items = $(
              '#' +
                selectAreaMatch[1] +
                ' .tabs-panel-active .menu-item-title input',
            );
            if (items.length === items.filter(':checked').length)
              items.removeAttr('checked');
            else items.prop('checked', true);
            return false;
          }
          ...
          if (e.target.id && 'submit-customlinkdiv' == e.target.id)
            ...
            $('#' + e.target.id.replace(/submit-/, '')).addSelectedToMenu(
              api.addMenuItemToBottom,
            );
          return false;
        }
      });
    ...
    processQuickSearchQueryResponse: function(resp, req, panel) {
      ...
      if (!$items.length) {
        $('.categorychecklist', panel).html(
          '<li><p>' + navMenuL10n.noResultsFound + '</p></li>',
        );
      ...
      $items.each(function() {
        ...
        if (matched && matched[1]) {
          newID = matched[1];
          while (
            form.elements['menu-item[' + newID + '][menu-item-type]'] ||
            takenIDs[newID]
          ) {
            newID--;
          }
