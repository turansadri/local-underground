  ...
  var imageEdit = (window.imageEdit = {
      ...
      img = $('<img id="image-preview-' + postid + '" alt="" />')
        .on('load', { history: data.history }, function(event) {
          ...
          if (
            ...
            $(
              ...
              '#imgedit-panel-' + postid,
            ).removeAttr('disabled');
          } else {
            $('input.imgedit-submit-btn', '#imgedit-panel-' + postid).prop(
              'disabled',
              true,
            );
          }
        ...
        .on('error', function() {
          $('#imgedit-crop-' + postid)
            .empty()
            .append(
              '<div class="error"><p>' + imageEditL10n.error + '</p></div>',
            );
          t.toggleEditor(postid, 0);
        })
    ...
    save: function(postid, nonce) {
      ...
      data = {
        ...
        history: history,
        target: target,
        context: $('#image-edit-context').length
          ? $('#image-edit-context').val()
          : null,
        do: 'save',
      };
      ...
      $.post(ajaxurl, data, function(r) {
        ...
        if (ret.error) {
          $('#imgedit-response-' + postid).html(
            '<div class="error"><p>' + ret.error + '</p></div>',
          );
        ...
        if (ret.msg) {
          $('#imgedit-response-' + postid).html(
            '<div class="updated"><p>' + ret.msg + '</p></div>',
          );
        }
    ...
    imgLoaded: function(postid) {
      ...
      $('.imgedit-wrap .imgedit-help-toggle')
        .eq(0)
        .focus();
    },
    ...
    rotate: function(angle, postid, nonce, t) {
      ...
      this.addStep(
        { r: { r: angle, fw: this.hold.h, fh: this.hold.w } },
        postid,
        nonce,
      );
    },
    ...
    flip: function(axis, postid, nonce, t) {
      ...
      this.addStep(
        { f: { f: axis, fw: this.hold.w, fh: this.hold.h } },
        postid,
        nonce,
      );
    },
