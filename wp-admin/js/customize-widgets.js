...
(function(wp, $) {
  ...
  api.Widgets.AvailableWidgetsPanelView = wp.Backbone.View.extend(
    /** @lends wp.customize.Widgets.AvailableWidgetsPanelView.prototype */ {
      ...
      initialize: function() {
        ...
        $(
          '#customize-controls, #available-widgets .customize-section-title',
        ).on('click keydown', function(e) {
          var isAddNewBtn = $(e.target).is(
      ...
      updateList: function() {
        this.collection.each(function(widget) {
          ...
          widgetTpl.toggle(
            widget.get('search_matched') && !widget.get('is_disabled'),
          );
      ...
      submit: function(widgetTpl) {
        ...
        widgetFormControl = this.currentSidebarControl.addWidget(
          widget.get('id_base'),
        );
  ...
  api.Widgets.formSyncHandlers = {
    ...
    rss: function(e, widget, newForm) {
      var oldWidgetError = widget.find('.widget-error:first'),
        newWidgetError = $('<div>' + newForm + '</div>').find(
          '.widget-error:first',
        );
    ...
    /** @lends wp.customize.Widgets.WidgetControl.prototype */ {
      ...
      embedWidgetContent: function() {
        ...
        $(document).trigger('widget-added', [
          control.container.find('.widget:first'),
        ]);
      },
      ...
      _setupWideWidget: function() {
        ...
        if (
          !this.params.is_wide ||
          $(window).width() <=
            640 /* max-width breakpoint in customize-controls.css */
        ) {
          return;
        }
      ...
      _setupReorderUI: function() {
        ...
        selectSidebarItem = function(li) {
          ...
          self.container
            .find('.move-widget-btn')
            .prop('disabled', isSelfSidebar);
        };
        ...
        $moveWidgetArea = $(
          template({
            sidebars: _(api.Widgets.registeredSidebars.toArray()).pluck(
              'attributes',
            ),
          }),
        );
        ...
        updateAvailableSidebars();
        api.Widgets.registeredSidebars.on(
          'change:is_rendered',
          updateAvailableSidebars,
        );
          ...
          .on('click keypress', function(event) {
            ...
            if ($(this).is('.move-widget')) {
              ...
              if (
                (isMoveUp && i === 0) ||
                (isMoveDown &&
                  i === self.getSidebarWidgetsControl().setting().length - 1)
              ) {
                return;
              }
        ...
        this.container.find('.move-widget-btn').click(function() {
          ...
          var oldSidebarId = self.params.sidebar_id,
            newSidebarId = self.container
              .find('.widget-area-select li.selected')
              .data('id'),
          ...
          oldSidebarWidgetsSetting = api(
            'sidebars_widgets[' + oldSidebarId + ']',
          );
          newSidebarWidgetsSetting = api(
            'sidebars_widgets[' + newSidebarId + ']',
          );
          oldSidebarWidgetIds = Array.prototype.slice.call(
            oldSidebarWidgetsSetting(),
          );
          newSidebarWidgetIds = Array.prototype.slice.call(
            newSidebarWidgetsSetting(),
          );
      ...
      _setupUpdateUI: function() {
        ...
        formSyncHandler =
          api.Widgets.formSyncHandlers[this.params.widget_id_base];
        if (formSyncHandler) {
      ...
      _setInputState: function(input, state) {
        ...
        if (input.is(':radio, :checkbox')) {
          ...
          input.find('option').each(function() {
            $(this).prop(
              'selected',
              -1 !== _.indexOf(state, String(this.value)),
            );
          });
        } else {
          input.val(state);
      ...
      updateWidget: function(args) {
        ...
        if (instanceOverride) {
          data +=
            '&' +
            $.param({
              sanitized_widget_setting: JSON.stringify(instanceOverride),
            });
        } else {
        ...
        jqxhr.done(function(r) {
          ...
          if (r.success) {
            ...
            hasSameInputsInResponse =
              self._getInputsSignature($inputs) ===
              self._getInputsSignature($sanitizedInputs);
            ...
            if (hasSameInputsInResponse && self.liveUpdateMode) {
              $inputs.each(function(i) {
                ...
                canUpdateState =
                  ...
                  (args.ignoreActiveElement ||
                    !$input.is(document.activeElement));
                if (canUpdateState) {
            ...
            isChanged =
              !isLiveUpdateAborted &&
              !_(self.setting()).isEqual(r.data.instance);
            if (isChanged) {
            ...
            if (completeCallback) {
              completeCallback.call(self, null, {
                noChange: !isChanged,
                ajaxFinished: true,
              });
            }
        ...
        panel.deferred.embedded.done(function() {
          ...
          shouldShowNotice = function() {
            ...
            if (0 === activeSectionCount) {
              ...
              return (
                activeSectionCount !==
                api.Widgets.data.registeredSidebars.length
              );
            }
          };
          ...
          api.bind('pane-contents-reflowed', function() {
            var duration =
              'resolved' === api.previewer.deferred.active.state() ? 'fast' : 0;
            updateNotice();
            if (shouldShowNotice()) {
  ...
  api.Widgets.SidebarSection = api.Section.extend(
    /** @lends wp.customize.Widgets.SidebarSection.prototype */ {
      ...
      ready: function() {
        ...
        registeredSidebar = api.Widgets.registeredSidebars.get(
          section.params.sidebarId,
        );
  ...
  api.Widgets.SidebarControl = api.Control.extend(
    /** @lends wp.customize.Widgets.SidebarControl.prototype */ {
      ...
      ready: function() {
        ...
        this.$sectionContent = this.container.closest(
      ...
      _setupModel: function() {
        ...
        this.setting.bind(function(newWidgetIds, oldWidgetIds) {
          ...
          newWidgetIds = _(newWidgetIds).filter(function(newWidgetId) {
            ...
            return !!api.Widgets.availableWidgets.findWhere({
              id_base: parsedWidgetId.id_base,
            });
          });
          ...
          _(removedWidgetIds).each(function(removedWidgetId) {
            ...
            setTimeout(function() {
              ...
              removedControl = api.Widgets.getWidgetFormControlForWidget(
                removedWidgetId,
              );
              ...
              wasDraggedToAnotherSidebar =
                ...
                !$.contains(
                  self.$sectionContent[0],
                  removedControl.container[0],
                );
              ...
              widget = api.Widgets.availableWidgets.findWhere({
                id_base: removedIdBase,
              });
      ...
      _setupSortable: function() {
        ...
        this.$sectionContent.sortable({
          ...
          tolerance: 'pointer',
          connectWith:
            '.accordion-section-content:has(.customize-control-sidebar_widgets)',
          update: function() {
      ...
      _applyCardinalOrderClassNames: function() {
        ...
        _.each(this.setting(), function(widgetId) {
          var widgetControl = api.Widgets.getWidgetFormControlForWidget(
            widgetId,
          );
          if (widgetControl) {
        ...
        if (
          ...
          (1 === api.Widgets.registeredSidebars.length &&
            widgetControls.length <= 1)
        ) {
      ...
      addWidget: function(widgetId) {
        ...
        if (!isExistingWidget) {
          settingArgs = {
            transport: api.Widgets.data.selectiveRefreshableWidgets[
              widget.get('id_base')
            ]
              ? 'postMessage'
              : 'refresh',
  ...
  api.bind('ready', function() {
    ...
    api.previewer.bind(
      'highlight-widget-control',
      api.Widgets.highlightWidgetFormControl,
    );

    // Open and focus widget control
    api.previewer.bind(
      'focus-widget-control',
      api.Widgets.focusWidgetFormControl,
    );
  });
  ...
  api.Widgets.getWidgetFormControlForWidget = function(widgetId) {
    ...
    api.control.each(function(control) {
      if (
        control.params.type === 'widget_form' &&
        control.params.widget_id === widgetId
      ) {
        foundControl = control;
      }
    });
  ...
  $(document).on('widget-added', function(event, widgetContainer) {
    ...
    widgetControl = api.control(
      'widget_nav_menu[' + String(parsedWidgetId.number) + ']',
    );
    if (!widgetControl) {
      return;
    }
