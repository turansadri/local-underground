...
(function(exports, $) {
    ...
    /** @lends wp.customize.Notifications.prototype */ {
      ...
      get: function(args) {
        ...
        if (params.sort) {
          ...
          notifications.sort(function(a, b) {
            ...
            return (
              collection._addedOrder[b.code] - collection._addedOrder[a.code]
            ); // Show newer notifications higher.
          });
        }
      ...
      render: function() {
        ...
        if (hasOverlayNotification !== hadOverlayNotification) {
          ...
          collection.container.toggleClass(
            'has-overlay-notifications',
            hasOverlayNotification,
          );
      ...
      constrainFocus: function constrainFocus(event) {
        ...
        if (
          ...
          focusableElements.last().is(event.target) &&
          !event.shiftKey
        ) {
          event.preventDefault();
          focusableElements.first().focus();
        } else if (
          focusableElements.first().is(event.target) &&
          event.shiftKey
        ) {
          event.preventDefault();
  ...
  api.dirtyValues = function dirtyValues(options) {
    ...
    api.each(function(setting) {
      ...
      if (
        ...
        (options && options.unsaved) &&
        (_.isUndefined(settingRevision) ||
          settingRevision <= api._lastSavedRevision)
      ) {
        return;
      }
  ...
  api.requestChangesetUpdate = function requestChangesetUpdate(changes, args) {
    ...
    if (submittedArgs.status) {
      return deferred
        .reject({ code: 'illegal_status_in_changeset_update' })
        .promise();
    }
    ...
    if (submittedArgs.date && submittedArgs.autosave) {
      return deferred
        .reject({ code: 'illegal_autosave_with_date_gmt' })
        .promise();
    }
    ...
    request.done(function requestChangesetUpdateDone(data) {
      ...
      api._lastSavedRevision = Math.max(
        api._latestRevision,
        api._lastSavedRevision,
      );
  ...
  api.utils.getCurrentTimestamp = function getCurrentTimestamp() {
    ...
    timestampDifferential =
      currentClientTimestamp - api.settings.initialClientTimestamp;
    timestampDifferential +=
  ...
  Container = api.Class.extend(
    /** @lends wp.customize~Container.prototype */ {
      ...
      initialize: function(id, options) {
        ...
        container.container = container.container.add(
      ...
      getNotificationsContainerElement: function() {
        var container = this;
        return container.contentContainer.find(
      ...
      isContextuallyActive: function() {
        throw new Error(
          'Container.isContextuallyActive() must be overridden in a subclass.',
        );
      },
      ...
      onChangeActive: function(active, args) {
        ...
        duration =
          'resolved' === api.previewer.deferred.active.state()
            ? args.duration
            : 0;
      ...
      _animateChangeExpanded: function(completeCallback) {
        ...
        if (!construct.panel || '' === construct.panel()) {
          ...
          api
            ...
            .contentContainer.hasClass('skip-transition')
        ) {
          transitionParentPane = true;
        } else {
      ...
      getContainer: function() {
        ...
        if (0 !== $('#tmpl-' + container.templateSelector).length) {
          ...
          template = wp.template(
            'customize-' + container.containerType + '-default',
          );
        }
        if (template && container.container) {
          return $.trim(
            template(_.extend({ id: container.id }, container.params)),
          );
        }
      ...
      getContent: function() {
        var construct = this,
          ...
          content = container
            .find('.accordion-section-content, .control-panel-content')
            .first(),
  ...
  api.Section = Container.extend(
    /** @lends wp.customize.Section.prototype */ {
      ...
      attachEvents: function() {
        ...
        section.container
          ...
          .on('click', function() {
            ...
            content.slideToggle(
              section.defaultExpandedArguments.duration,
              function() {
      ...
      onChangeExpanded: function(expanded, args) {
        var section = this,
          container = section.headContainer.closest(
    ...
    /** @lends wp.customize.ThemesSection.prototype */ {
      ...
      attachEvents: function() {
        ...
        if ('local' === section.params.filter_type) {
          ...
          section.container.on('input', '.wp-filter-search-themes', function(
            event,
          ) {
        ...
        section.contentContainer.on('click', '.feature-filter-toggle', function(
          e,
        ) {
        ...
        section.contentContainer.on(
          'click',
          '.no-themes-local .search-dotorg-themes',
          function() {
        ...
        function updateSelectedState() {
          var el = section.headerContainer.find(
            '.customize-themes-section-title',
          );
        ...
        api.bind('ready', function() {
          section.contentContainer = section.container.find(
            '.customize-themes-section',
          );
          section.contentContainer.appendTo(
      ...
      onChangeExpanded: function(expanded, args) {
        ...
        var section = this,
          container = section.contentContainer.closest(
            '.customize-themes-full-container',
          );
          ...
          api.section.each(function(otherSection) {
            ...
            if (otherSection !== section) {
              ...
              if ('themes' === otherSection.params.type) {
                searchTerm = otherSection.contentContainer
                  .find('.wp-filter-search')
                  .val();
                section.contentContainer
                  .find('.wp-filter-search')
                  .val(searchTerm);
        ...
        request.done(function(data) {
          ...
          if (0 !== themes.length) {
            ...
            if (1 === page) {
              ...
              if ('local' !== section.params.filter_type) {
                wp.a11y.speak(
                  api.settings.l10n.themeSearchResults.replace(
                    '%d',
                    data.info.results,
                  ),
                );
      ...
      filterSearch: function(term) {
        var count = 0,
          ...
          section = this,
          noFilter =
            api.section.has('wporg_themes') &&
            'remote' !== section.params.filter_type
              ? '.no-themes-local'
              : '.no-themes',
          controls = section.controls(),
          terms;
      ...
      filtersChecked: function() {
        ...
        if (0 === tags.length) {
          tags = '';
          section.contentContainer
            .find('.feature-filter-toggle .filter-count-0')
            .show();
          section.contentContainer
            .find('.feature-filter-toggle .filter-count-filters')
            .hide();
        } else {
          section.contentContainer
      ...
      renderScreenshots: function() {
        ...
        if (
          null === section.screenshotQueue ||
          0 === section.screenshotQueue.length
        ) {
          // Add controls that haven't had their screenshots rendered.
          section.screenshotQueue = _.filter(section.controls(), function(
            control,
          ) {
        ...
        section.screenshotQueue = _.filter(section.screenshotQueue, function(
          control,
        ) {
      ...
      getVisibleCount: function() {
        return this.contentContainer.find('li.customize-control:visible')
          .length;
      },
      ...
      updateCount: function(count) {
        ...
        if (0 === count) {
          ...
          wp.a11y.speak(
            api.settings.l10n.announceThemeCount.replace('%d', count),
          );
        }
      },
      ...
      getNextTheme: function() {
        var section = this,
          ...
          i;
        control = api.control(
          section.params.action + '_theme_' + section.currentTheme,
        );
      ...
      showDetails: function(theme, callback) {
        ...
        wp.a11y.speak(
          api.settings.l10n.announceThemeDetails.replace('%s', theme.name),
        );
        if (callback) {
          callback();
        }
    ...
    /** @lends wp.customize.Panel.prototype */ {
        ...
        meta
          ...
          .on('click', function() {
            ...
            if (meta.hasClass('open')) {
              ...
              content.slideUp(
                panel.defaultExpandedArguments.duration,
                function() {
                  content.trigger('toggled');
                },
              );
      ...
      onChangeExpanded: function(expanded, args) {
        ...
        var panel = this,
          ...
          container = accordionSection.closest(
            '.wp-full-overlay-sidebar-content',
          ),
      ...
      renderContent: function() {
        ...
        if (template && panel.headContainer) {
          panel.contentContainer.html(
            template(_.extend({ id: panel.id }, panel.params)),
          );
        }
      },
    },
  ...
  api.ThemesPanel = api.Panel.extend(
    /** @lends wp.customize.ThemsPanel.prototype */ {
      ...
      attachEvents: function() {
        ...
        if (
          api.settings.theme._canInstall &&
      ...
      installTheme: function(event) {
        ...
        onInstallSuccess = function(response) {
          ...
          if (preview) {
            ...
            api.control.each(function(control) {
              if (
                'theme' === control.params.type &&
                control.params.theme.id === response.slug
              ) {
            ...
            themeControl = new api.controlConstructor.theme(
              'installed_theme_' + theme.id,
              {
                type: 'theme',
  ...
  api.Control = api.Class.extend(
    /** @lends wp.customize.Control.prototype */ {
      ...
      initialize: function(id, options) {
        ...
        control.id = id;
        control.selector =
          '#customize-control-' + id.replace(/\]/g, '').replace(/\[/g, '-'); // Deprecated, likely dead code from time before #28709.
        if (control.params.content) {
        ...
        if (control.params.templateId) {
          ...
          control.templateSelector =
            'customize-control-' + control.params.type + '-content';
        }
        ...
        control.priority.set(
          isNaN(control.params.priority) ? 10 : control.params.priority,
        );
        control.active.set(control.params.active);

        api.utils.bubbleChildValueChanges(control, [
          'section',
          'priority',
          'active',
        ]);
        ...
        _.each(settings, function(value, key) {
          ...
          if (
            _.isObject(value) &&
            _.isFunction(value.extended) &&
            value.extended(api.Value)
          ) {
      ...
      getNotificationsContainerElement: function() {
        ...
        if (control.container.hasClass('customize-control-nav_menu_item')) {
          control.container
            .find('.menu-item-settings:first')
            .prepend(notificationsContainer);
        } else if (
          control.container.hasClass('customize-control-widget_form')
        ) {
          control.container
            .find('.widget-inside:first')
            .prepend(notificationsContainer);
        } else {
      ...
      setupNotifications: function() {
          ...
          setting.notifications.bind('add', function(settingNotification) {
            ...
            control.notifications.add(
              new api.Notification(
                setting.id + ':' + settingNotification.code,
                params,
              ),
            );
          });
          setting.notifications.bind('remove', function(settingNotification) {
            control.notifications.remove(
              setting.id + ':' + settingNotification.code,
            );
          });
        });
        ...
        renderNotificationsIfVisible = function() {
          ...
          if (
            !sectionId ||
            (api.section.has(sectionId) && api.section(sectionId).expanded())
          ) {
        ...
        onSectionAssigned = function(newSectionId, oldSectionId) {
          if (oldSectionId && api.section.has(oldSectionId)) {
            api
              ...
              .expanded.unbind(renderNotificationsIfVisible);
          }
          if (newSectionId) {
        ...
        control.notifications.bind(
          'change',
          _.debounce(renderNotificationsIfVisible),
        );
      },
      ...
      renderNotifications: function() {
        ...
        if (!control.notificationsTemplate) {
          control.notificationsTemplate = wp.template(
            'customize-control-notifications',
          );
        }
      ...
      addNewPage: function() {
        ...
        promise.done(function(data) {
          ...
          $content = $('#available-menu-items-post_type-page').find(
        ...
        function setAttachmentDataAndRenderContent(value) {
          ...
          if (control.extended(api.UploadControl)) {
            ...
            if (_.isNaN(value) || value <= 0) {
              ...
              control.params.attachment.id === value
            ) {
              hasAttachmentData.resolve();
            }
          }
  ...
  api.HeaderControl = api.Control.extend(
    /** @lends wp.customize.HeaderControl.prototype */ {
      ready: function() {
        ...
        wp.media.controller.Cropper.prototype.defaults.doCropArgs.wp_customize =
          'on';
      ...
      getInitialHeaderImage: function() {
        ...
        var currentHeaderObject = _.find(_wpCustomizeHeader.uploads, function(
          imageObj,
        ) {
          return (
  ...
  api.ThemeControl = api.Control.extend(
    /** @lends wp.customize.ThemeControl.prototype */ {
      ...
      ready: function() {
        ...
        control.container.on('click keydown touchend', '.theme', function(
          event,
        ) {
          var section;
          if (api.utils.isKeydownButNotEnterEvent(event)) {
      ...
      filter: function(terms) {
        ...
        if (control.params.theme.name.toLowerCase() === terms.join(' ')) {
          ...
          matchCount =
            matchCount + 10 * (haystack.split(terms.join(' ')).length - 1);

          // Search for each term individually (as whole-word and partial match) and sum weighted match counts.
          _.each(terms, function(term) {
            matchCount =
              matchCount + 2 * (haystack.split(term + ' ').length - 1); // Whole-word, double-weighted.
  ...
  api.CodeEditorControl = api.Control.extend(
    /** @lends wp.customize.CodeEditorControl.prototype */ {
      ...
      initialize: function(id, options) {
        ...
        control.notifications.bind('add', function(notification) {
          ...
          notification.templateId =
            'customize-code-editor-lint-error-notification';
      ...
      initSyntaxHighlightingEditor: function(codeEditorSettings) {
        ...
        control.editor.codemirror.on('keydown', function onKeydown(
          codemirror,
          event,
        ) {
          var escKeyCode = 27;
        ...
        control.deferred.codemirror.resolveWith(control, [
          control.editor.codemirror,
        ]);
      },
      ...
      onTabPrevious: function onTabPrevious() {
        ...
        if (0 === controlIndex) {
          ...
          controls[controlIndex - 1].contentContainer
      ...
      onUpdateErrorNotice: function onUpdateErrorNotice(errorAnnotations) {
        ...
        if (0 !== errorAnnotations.length) {
          if (1 === errorAnnotations.length) {
            ...
            message = api.l10n.customCssError.plural.replace(
              '%d',
    ...
    /** @lends wp.customize.DateTimeControl.prototype */ {
      ...
      parseDateTime: function parseDateTime(datetime) {
        ...
        if (control.params.includeTime && control.params.twelveHourFormat) {
          ...
          date.hour =
            date.hour % midDayHour
              ? String(date.hour % midDayHour)
              : String(midDayHour);
      ...
      isFutureDate: function isFutureDate() {
        var control = this;
        return (
          0 < api.utils.getRemainingTime(control.convertInputDateToString())
        );
      },
       ...
       */
      convertHourToTwentyFourHourFormat: function convertHour(
       ...
       */
      toggleFutureDateNotification: function toggleFutureDateNotification(
        notify,
      ) {
        var control = this,
          notificationCode,
  ...
  api.PreviewLinkControl = api.Control.extend(
    /** @lends wp.customize.PreviewLinkControl.prototype */ {
      ...
      updatePreviewLink: function updatePreviewLink() {
        ...
        control.previewElements.url.element
          .parent()
          .toggleClass('disabled', unsavedDirtyValues);
        control.previewElements.button.element.prop(
          'disabled',
          unsavedDirtyValues,
        );
        ...
        params = _.extend(
          api.utils.parseQueryString(urlParser.search.substr(1)),
          {
            customize_changeset_uuid:
              previewFrame.query.customize_changeset_uuid,
            customize_theme: previewFrame.query.customize_theme,
            customize_messenger_channel:
              previewFrame.query.customize_messenger_channel,
          },
        );
  ...
  api.Previewer = api.Messenger.extend(
    /** @lends wp.customize.Previewer.prototype */ {
      ...
      initialize: function(params, options) {
              ...
              if (isProcessingComplete()) {
                ...
                refreshOnceProcessingComplete = function() {
                  if (isProcessingComplete()) {
                    ...
                    api
                      .state('processing')
                      .unbind(refreshOnceProcessingComplete);
                  }
                };
        ...
        previewer.deferred.active.done(
          _.bind(previewer.keepPreviewAlive, previewer),
        );
      ...
      ready: function(data) {
        ...
        synced['settings-modified-while-loading'] =
          previewer.settingsModifiedWhileLoading;
        if (
          'resolved' !== previewer.deferred.active.state() ||
          previewer.loading
        ) {
          synced.scroll = previewer.scroll;
        }
        synced['edit-shortcut-visibility'] = api
        ...
        _(constructs).each(function(activeConstructs, type) {
          api[type].each(function(construct, id) {
            var isDynamicallyCreated = _.isUndefined(
              api.settings[type + 's'][id],
            );
      ...
      keepPreviewAlive: function keepPreviewAlive() {
        ...
        scheduleKeepAliveCheck = function() {
          timeoutId = setTimeout(
            handleMissingKeepAlive,
            api.settings.timeouts.keepAliveCheck,
          );
        };
        ...
        if (_.isObject(validity)) {
          _.each(validity, function(params, code) {
            ...
            notification = new api.Notification(
              code,
              _.extend({ fromServer: true }, params),
            );
  ...
  api.reflowPaneContents = _.bind(function() {
    ...
    api.panel.each(function(panel) {
      ...
      if (
        !api.utils.areElementListsEqual(
          sectionHeadContainers,
          appendContainer.children('[id]'),
        )
      ) {
        _(sections).each(function(section) {
  ...
  $(function() {
    ...
    if (
      !$.support.postMessage ||
      (!$.support.cors && api.settings.isCrossDomain)
    ) {
      return;
    }

    if (null === api.PreviewFrame.prototype.sensitivity) {
      api.PreviewFrame.prototype.sensitivity =
        api.settings.timeouts.previewFrameSensitivity;
    }
    if (null === api.Previewer.prototype.refreshBuffer) {
      api.Previewer.prototype.refreshBuffer =
        api.settings.timeouts.windowRefresh;
    }
    ...
    api.section('publish_settings', function(section) {
      ...
      isSectionActive = function() {
        ...
        if (
          api.state('trashing').get() ||
          'trash' === api.state('changesetStatus').get()
        ) {
          return false;
        }
        if (
      ...
      section.contentContainer
        .find('.customize-action')
        .text(api.l10n.updating);
      section.contentContainer
        .find('.customize-section-back')
        .removeAttr('tabindex');
      ...
      section.expanded.bind(function(isExpanded) {
        ...
        if (
          '' === defaultChangesetStatus ||
          'auto-draft' === defaultChangesetStatus
        ) {
          defaultChangesetStatus = 'publish';
        }

        if (
          api.state('selectedChangesetStatus').get() !== defaultChangesetStatus
        ) {
      ...
      updateTimeArrivedPoller = function() {
        ...
        if (shouldPoll && !pollInterval) {
          pollInterval = setInterval(function() {
            var remainingTime = api.utils.getRemainingTime(
              api.state('changesetDate').get(),
            );
    ...
    $('.customize-info')
      ...
      .on('click', function() {
        ...
        if (section.hasClass('open')) {
          ...
          content.slideUp(
            api.Panel.prototype.defaultExpandedArguments.duration,
            function() {
              content.trigger('toggled');
            },
          );
        ...
        save: function(args) {
          ...
          submit = function() {
            ...
            api.control.each(function(control) {
              if (
                !control.setting ||
                (!control.setting.id && control.active.get())
              ) {
                control.notifications.each(function(notification) {
                  if ('error' === notification.type) {
             ...
             */
            query = $.extend(
              previewer.query({ excludeCustomizedSaved: false }),
              {
            ...
            request.fail(function(response) {
              ...
              if (
                'changeset_already_published' === response.code &&
                response.next_changeset_uuid
              ) {
    ...
    (function(state) {
      ...
      state.bind('change', function() {
        ...
        canSave =
          ...
          (!activated() ||
            !saved() ||
            (changesetStatus() !== selectedChangesetStatus() &&
              '' !== changesetStatus()) ||
      ...
      defaultSelectedChangesetStatus = api.settings.changeset
      ...
      selectedChangesetStatus(
        '' === api.settings.changeset.status ||
          'auto-draft' === api.settings.changeset.status
          ? defaultSelectedChangesetStatus
          : api.settings.changeset.status,
      );
    ...
    (function checkAndDisplayLockNotice() {
      var LockedNotification = api.OverlayNotification.extend(
        /** @lends wp.customize~LockedNotification.prototype */ {
          ...
          initialize: function(code, params) {
            ...
            api.OverlayNotification.prototype.initialize.call(
              notification,
              _code,
              _params,
            );
      ...
      $(document).on('heartbeat-send.update_lock_notice', function(
        event,
        data,
      ) {
      ...
      $(document).on('heartbeat-tick.update_lock_notice', function(
        event,
        data,
      ) {
        var notification,
        ...
        if (
          notification &&
          notification.lockUser.id !== api.settings.changeset.lockUser.id
        ) {
    ...
    body.on('keydown', function(event) {
      ...
      if (
        ...
        !$.contains($('#customize-controls')[0], event.target)
      ) {
        return;
      }
      ...
      collapsedObject =
        expandedControls[0] || expandedSections[0] || expandedPanels[0];
      if (collapsedObject) {
      ...
      changeContainer = function(container) {
        ...
        if (!newInstance) {
          if (
            !expandedSection &&
            expandedPanel &&
            expandedPanel.contentContainer
          ) {
            newInstance = expandedPanel;
          } else if (
            !expandedPanel &&
            expandedSection &&
            expandedSection.contentContainer
          ) {
        ...
        if ('page' === showOnFront()) {
          ...
          if (setting === pageForPosts && pageForPostsId > 0) {
            api.previewer.previewUrl.set(
              api.settings.url.home + '?page_id=' + pageForPostsId,
            );
          }
        }
    ...
    (function() {
      ...
      sectionReady.done(function setupSectionDescription(section) {
        ...
        control.container
          .find('.customize-control-title:first')
    ...
    api.state('paneVisible').bind(function(isPaneVisible) {
      ...
      if (window.matchMedia) {
        isMobileScreen = window.matchMedia('screen and ( max-width: 640px )')
          .matches;
      } else {
    ...
    function startAutosaving() {
      ...
      $(document).on(
        'visibilitychange.wp-customize-changeset-update',
        function() {
          if (document.hidden) {
            updateChangesetWithReschedule();
          }
