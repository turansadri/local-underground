...
(function($, window, undefined) {
  ...
  $availableStructureTags.on('click', function() {
    ...
    if (-1 !== permalinkStructureValue.indexOf(textToAppend)) {
      permalinkStructureValue = permalinkStructureValue.replace(
        textToAppend + '/',
        '',
      );

      $permalinkStructure.val(
        '/' === permalinkStructureValue ? '' : permalinkStructureValue,
      );
    ...
    if (permalinkStructureFocused && $permalinkStructure[0].setSelectionRange) {
      newSelectionStart = (
        permalinkStructureValue.substr(0, selectionStart) + textToAppend
      ).length;
      $permalinkStructure[0].setSelectionRange(
        newSelectionStart,
        newSelectionStart,
      );
  ...
  $document.ready(function() {
    ...
    if (
      ...
      $adminmenu
        ...
        .on(mobileEvent + '.wp-mobile-hover', function(event) {
    ...
    $body.on('click', 'tbody > tr > .check-column :checkbox', function(event) {
      ...
      if (event.shiftKey) {
        ...
        if (0 < first && 0 < last && first != last) {
          sliced =
            last > first
              ? checks.slice(first, last)
              : checks.slice(last, first);
      ...
      if (height.menu + height.adminbar > height.window) {
        ...
        if (windowPos > lastScrollPosition) {
          ...
          if (pinnedMenuTop) {
            ...
            menuTop =
              $adminMenuWrap.offset().top -
              height.adminbar -
              (windowPos - lastScrollPosition);

            if (
              menuTop + height.menu + height.adminbar <
              windowPos + height.window
            ) {
              menuTop =
                windowPos + height.window - height.menu - height.adminbar;
            }
            ...
            !pinnedMenuBottom &&
            $adminMenuWrap.offset().top + height.menu <
              windowPos + height.window
          ) {
          ...
          if (pinnedMenuBottom) {
            ...
            !pinnedMenuTop &&
            $adminMenuWrap.offset().top >= windowPos + height.adminbar
          ) {
            // Pin it to the top.
            pinnedMenuTop = true;
          ...
          menuTop =
            windowPos + height.window - height.menu - height.adminbar - 1;
    ...
    window.wpResponsive = {
      ...
      init: function() {
        ...
        $('#wp-admin-bar-menu-toggle').on('click.wp-responsive', function(
          event,
        ) {
          event.preventDefault();
        ...
        // Add menu events.
        $adminmenu.on('click.wp-responsive', 'li.wp-has-submenu > a', function(
          event,
        ) {
          if (!$adminmenu.data('wp-responsive')) {
            return;
        ...
        self.trigger();
        $document.on(
          'wp-window-resized.wp-responsive',
     ...
     * @returns {void}
     */
    $document.on('wp-menu-state-set wp-collapse-menu', function(
      event,
      eventData,
    ) {
