...
(function($) {
    ...
    get: function(total, num) {
      ...
      $.post(ajaxurl, data, function(r) {
        ...
        $('#the-comment-list').append(
          '<tr><td colspan="2">' + wpAjax.broken + '</td></tr>',
        );
      });
  ...
  window.WPSetThumbnailID = function(id) {
    ...
    if (field.length > 0) {
      $('#meta\\[' + field.attr('id').match(/[0-9]+/) + '\\]\\[value\\]').text(
        id,
      );
    }
  };
        ...
        if (received.lock_error) {
          ...
          if (wrap.length && !wrap.is(':visible')) {
            ...
            if (received.lock_error.avatar_src) {
              avatar = $(
                '<img class="avatar avatar-64 photo" width="64" height="64" alt="" />',
              ).attr(
                'src',
                received.lock_error.avatar_src.replace(/&amp;/g, '&'),
              );
              wrap
...
jQuery(document).ready(function($) {
  var stamp,
    ...
    isMac = window.navigator.platform
      ...
      $('form#post')
        ...
        .on('submit.edit-post', function(event) {
          ...
          if ($button.attr('id') === 'publish') {
            $submitpost
              .find('#major-publishing-actions .spinner')
              .addClass('is-active');
          } else {
            $submitpost
              .find('#minor-publishing .spinner')
              .addClass('is-active');
          }
        });
    });
  ...
  if ($('#submitdiv').length) {
    ...
    updateVisibility = function() {
      ...
      if (
        $postVisibilitySelect.find('input:radio:checked').val() != 'password'
      ) {
        $('#password-span').hide();
      } else {
        $('#password-span').show();
    ...
    updateText = function() {
      ...
      if (
        $postVisibilitySelect.find('input:radio:checked').val() == 'private'
      ) {
        $('#publish').val(postL10n.update);
        if (0 === optPublish.length) {
          postStatus.append(
            '<option value="publish">' +
              postL10n.privatelyPublished +
              '</option>',
          );
        } else {
      ...
      if (
        ...
        if ($('option:selected', postStatus).val() == 'pending') {
          $('#save-post')
            .show()
            .val(postL10n.savePending);
        } else {
          $('#save-post')
            .show()
            .val(postL10n.saveDraft);
        }
      }
      return true;
    };
    ...
    $postVisibilitySelect
      ...
      .click(function(event) {
        ...
        $('#visibility-radio-' + $('#hidden-post-visibility').val()).prop(
          'checked',
          true,
        );
    ...
    $postVisibilitySelect.find('.save-post-visibility').click(function(event) {
      ...
      $('#post-visibility-display').html(
        postL10n[
          $postVisibilitySelect.find('input:radio:checked').val() + sticky
        ],
      );
      event.preventDefault();
    });
  ...
  (function() {
    ...
    function endDrag() {
      ...
      if (mce) {
        ...
        height =
          parseInt($('#content_ifr').css('height'), 10) + toolbarHeight - 28;
      } else {
        $textarea.focus();
    ...
    $('#post-formats-select input.post-format').on(
      ...
      function() {
        ...
        if (
          ...
          editor.dom.addClass(
            body,
            format == 'post-format-0' ? 'post-format-standard' : format,
          );
  ...
  if (
    $('#original_post_status').val() === 'auto-draft' &&
