...
(function($) {
  ...
  themes.view.Theme = wp.Backbone.View.extend({
    ...
    setNavButtonsState: function() {
      ...
      if (
        _.isUndefined(
          this.model.collection.at(this.model.collection.indexOf(current) + 1),
        )
      ) {
  ...
  themes.view.Details = wp.Backbone.View.extend({
    ...
    containFocus: function($el) {
      ...
      $el.on('keydown.wp-themes', function(event) {
        var $firstFocusable = $el
            .find('.theme-header button:not(.disabled)')
            .first(),
    ...
    navigation: function() {
      ...
      if (
        this.model.cid ===
        this.model.collection.at(this.model.collection.length - 1).cid
      ) {
        this.$el
          .find('.right')
          .addClass('disabled')
  ...
  themes.view.Preview = themes.view.Details.extend({
    ...
    close: function() {
      ...
      if (themes.router.selectedTab) {
        themes.router.navigate(
          themes.router.baseUrl('?browse=' + themes.router.selectedTab),
        );
  ...
  themes.view.Themes = wp.Backbone.View.extend({
    ...
    expand: function(id) {
      ...
      themes.router.navigate(
        themes.router.baseUrl(themes.router.themePath + this.model.id),
      );
  ...
  themes.view.InstallerSearch = themes.view.Search.extend({
    ...
    doSearch: function(value) {
      ...
      // Set route
      themes.router.navigate(
        themes.router.baseUrl(
          themes.router.searchPath + encodeURIComponent(value),
        ),
        { replace: true },
      );
    },
  ...
  themes.view.Installer = themes.view.Appearance.extend({
    ...
    onSort: function(event) {
      ...
      themes.router.navigate(
        themes.router.baseUrl(themes.router.browsePath + sort),
      );
    },
    ...
    routes: function() {
      ...
      themes.router.on('route:preview', function(slug) {
        ...
        if (self.view.view.theme && self.view.view.theme.preview) {
          self.view.view.theme.model = self.view.collection.findWhere({
            slug: slug,
          });
      ...
      themes.router.on('route:sort', function(sort) {
        if (!sort) {
          sort = 'featured';
          themes.router.navigate(themes.router.baseUrl('?browse=featured'), {
            replace: true,
          });
        }
        self.view.sort(sort);
