...
(function($) {
  ...
  $.imgAreaSelect = function(img, options) {
    ...
    function areaMouseMove(event) {
      ...
      $box.css(
        'cursor',
        resize ? resize + '-resize' : options.movable ? 'move' : '',
      );
    ...
    function selectingMouseMove(event) {
      x2 =
        /w|e|^$/.test(resize) || aspectRatio ? evX(event) : viewX(selection.x2);
      y2 =
        /n|s|^$/.test(resize) || aspectRatio ? evY(event) : viewY(selection.y2);
    ...
    function movingMouseMove(event) {
      x1 = max(
        left,
        min(startX + evX(event), left + imgWidth - selection.width),
      );
      y1 = max(
        top,
        min(startY + evY(event), top + imgHeight - selection.height),
      );
