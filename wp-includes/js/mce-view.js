    ...
    /** @lends wp.mce.View.prototype */ {
      ...
      replaceMarkers: function() {
        this.getMarkers(function(editor, node) {
          ...
          if (
            !this.loader &&
            $(node).text() !== tinymce.DOM.decode(this.text)
          ) {
        ...
        if (
          ...
          this.getNodes(function(editor, node) {
            ...
            editor.undoManager.transact(function() {
              ...
              node.appendChild(
                _.isString(content)
                  ? editor.dom.createFragment(content)
                  : content,
              );
      ...
      setIframes: function(head, body, callback, rendered) {
        ...
        this.getNodes(function(editor, node) {
          ...
          tinymce.each(dom.$('link[rel="stylesheet"]', editorHead), function(
            link,
          ) {
            if (
              link.href &&
      ...
      update: function(text, editor, node, force) {
        _.find(views, function(view, type) {
          ...
          if (match) {
            ...
            wp.mce.views
              .createInstance(type, text, match.options, force)
              .render();
...
(function(window, views, media, $) {
  ...
  function verifyHTML(string) {
    ...
    serializer =
      serializer || new window.tinymce.html.Serializer(settings, schema);

    return serializer.serialize(
      parser.parse(string, { forced_root_block: false }),
    );
  }
  ...
  gallery = _.extend({}, base, {
    ...
    initialize: function() {
      var attachments = media.gallery.attachments(
          this.shortcode,
          ...
          self.render(
            self.template({
              ...
              columns: attrs.columns
                ? parseInt(attrs.columns, 10)
                : media.galleryDefaults.columns,
            }),
          );
        })
