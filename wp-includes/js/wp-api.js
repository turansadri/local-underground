...
(function(window, undefined) {
  ...
  wp.api.getCollectionByRoute = function(route) {
    return _.find(wp.api.collections, function(collection) {
      return (
        collection.prototype.route && route === collection.prototype.route.index
      );
    });
  };
   ...
   */
  wp.api.utils.extractRoutePart = function(
    route,
    part,
    versionString,
    reverse,
  ...
  wp.api.utils.decorateFromRoute = function(routeEndpoints, modelInstance) {
    ...
    _.each(routeEndpoints, function(routeEndpoint) {
      ...
      if (
        _.includes(routeEndpoint.methods, 'POST') ||
        _.includes(routeEndpoint.methods, 'PUT')
      ) {
   ...
   */
  wp.api.utils.addMixinsAndHelpers = function(
    model,
    modelClassName,
    loadingObjects,
  ) {
    var hasDate = false,
      ...
      buildModelGetter = function(
        ...
        if (embeddeds[embedSourcePoint]) {
          attributes = _.findWhere(embeddeds[embedSourcePoint], {
            id: modelId,
          });
        }
       ...
       */
      buildCollectionGetter = function(
        parentModel,
        collectionName,
        embedSourcePoint,
        embedIndex,
        ...
        if (
          !_.isUndefined(embedSourcePoint) &&
          !_.isUndefined(embeddeds[embedSourcePoint])
        ) {
        ...
        getObjects = new wp.api.collections[collectionName](
          properties,
          classProperties,
        );
            ...
            allTags.fetch({
              ...
              success: function(alltags) {
                ...
                _.each(tags, function(tag) {
                  newTag = new wp.api.models.Tag(
                    alltags.findWhere({ slug: tag }),
                  );
            ...
            allCategories.fetch({
              ...
              success: function(allcats) {
                ...
                _.each(categories, function(category) {
                  newCategory = new wp.api.models.Category(
                    allcats.findWhere({ slug: category }),
                  );
    ...
    if (
      !_.isUndefined(loadingObjects.collections[modelClassName + 'Revisions'])
    ) {
  ...
  wp.api.models.Schema = wp.api.WPApiBaseModel.extend(
    /** @lends Schema.prototype  */
    {
      ...
      initialize: function(attributes, options) {
        ...
        wp.api.WPApiBaseModel.prototype.initialize.call(
          model,
          attributes,
          options,
        );
        ...
        model.versionString =
          options.versionString || wpApiSettings.versionString;
      },
        ...
        if ('read' === method) {
          ...
          options.success = function(data, textStatus, request) {
            if (!_.isUndefined(request)) {
              self.state.totalPages = parseInt(
                request.getResponseHeader('x-wp-totalpages'),
                10,
              );
              self.state.totalObjects = parseInt(
...
(function() {
    ...
    /** @lends Endpoint.prototype */ {
      ...
      initialize: function() {
        ...
        if (model.get('schema')) {
          ...
          (_.isUndefined(wpApiSettings.cacheSchema) ||
            wpApiSettings.cacheSchema) &&
          sessionStorage.getItem(
            'wp-api-schema-model' +
              model.get('apiRoot') +
              model.get('versionString'),
          )
        ) {
      ...
      constructFromSchema: function() {
        ...
        modelRoutes = [];
        collectionRoutes = [];
        schemaRoot = routeModel
          .get('apiRoot')
          .replace(wp.api.utils.getRootUrl(), '');
        loadingObjects = {};
        ...
        _.each(modelRoutes, function(modelRoute) {
          ...
          if ('' !== parentName && parentName !== routeName) {
            ...
            modelClassName = wp.api.utils.capitalizeAndCamelCaseDashes(
              routeName,
            );
            modelClassName = mapping.models[modelClassName] || modelClassName;
            loadingObjects.models[
        ...
        _.each(collectionRoutes, function(collectionRoute) {
          ...
          var collectionClassName,
            modelClassName,
            routeName = collectionRoute.index.slice(
              collectionRoute.index.lastIndexOf('/') + 1,
            ),
            parentName = wp.api.utils.extractRoutePart(
          ...
          if (
            ...
            collectionClassName =
              ...
              wp.api.utils.capitalizeAndCamelCaseDashes(routeName);
            modelClassName =
              mapping.models[collectionClassName] || collectionClassName;
            collectionClassName =
              mapping.collections[collectionClassName] || collectionClassName;
            loadingObjects.collections[
              collectionClassName
            ] = wp.api.WPApiBaseCollection.extend({
              ...
              model: function(attrs, options) {
                return new loadingObjects.models[modelClassName](
                  attrs,
                  options,
                );
              },
            ...
            collectionClassName = wp.api.utils.capitalizeAndCamelCaseDashes(
              routeName,
            );
            modelClassName =
              mapping.models[collectionClassName] || collectionClassName;
  ...
  wp.api.init = function(args) {
    ...
    attributes.versionString =
      args.versionString || wpApiSettings.versionString || 'wp/v2/';
    ...
    if (!initializedDeferreds[attributes.apiRoot + attributes.versionString]) {
      ...
      endpoint.schemaConstructed.done(function(resolvedEndpoint) {
        ...
        wp.api.collections = _.extend(
          wp.api.collections,
          resolvedEndpoint.get('collections'),
        );
        deferred.resolve(resolvedEndpoint);
      });
      initializedDeferreds[
        attributes.apiRoot + attributes.versionString
      ] = promise;
    }
