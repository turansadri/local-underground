...
if (typeof jQuery != 'undefined') {
  ...
  jQuery(document).ready(function($) {
    ...
    refresh = function(i, el) {
      var node = $(el),
        tab = node.attr('tabindex');
      if (tab) node.attr('tabindex', '0').attr('tabindex', tab);
    };
    ...
    touchOpen = function(unbind) {
      adminbar.find('li.menupop').on('click.wp-mobile-hover', function(e) {
        ...
        if (
          el.parent().is('#wp-admin-bar-root-default') &&
          !el.hasClass('hover')
        ) {
          e.preventDefault();
     ...
     */
    touchClose = function() {
      var mobileEvent = /Mobile\/.+Safari/.test(navigator.userAgent)
        ? 'touchstart'
        : 'click';
    ...
    $('#wp-admin-bar-get-shortlink').click(function(e) {
      ...
      $(this)
        .addClass('selected')
        .children('.shortlink-input')
        .blur(function() {
          $(this)
            .parents('#wp-admin-bar-get-shortlink')
    ...
    $('#wpadminbar li.menupop > .ab-item')
      .bind('keydown.adminbar', function(e) {
        ...
        if (!wrap.length) wrap = $('#wpadminbar .quicklinks');
    ...
    adminbar.click(function(e) {
      if (
        e.target.id != 'wpadminbar' &&
        e.target.id != 'wp-admin-bar-top-secondary'
      ) {
        return;
      }
    ...
    if (
      navigator.userAgent &&
      document.body.className.indexOf('no-font-face') === -1 &&
      /Android (1.0|1.1|1.5|1.6|2.0|2.1)|Nokia|Opera Mini|w(eb)?OSBrowser|webOS|UCWEB|Windows Phone OS 7|XBLWP7|ZuneWP7|MSIE 7/.test(
        navigator.userAgent,
      )
    ) {
  ...
  (function(d, w) {
    ...
    var addEvent = function(obj, type, fn) {
      ...
      addHoverClass = function(t) {
            ...
            while (i--) {
              ...
              if (li != hovering)
                li.className = li.className
                  ? li.className.replace(rselected, '')
                  : '';
            }
          }
        }
        ...
        while (i--) {
          ...
          while (ancestorLength--) {
            if (ancestors[ancestorLength] == q[i][1]) inA = true;
          }

          if (!inA)
            q[i][1].className = q[i][1].className
              ? q[i][1].className.replace(hc, '')
              : '';
        }
      },
      /**
      ...
      clickShortlink = function(e) {
        ...
        while (true) {
          ...
          if (!t || t == d || t == aB) return;
          // Check if we've found the shortlink node.
          if (t.id && t.id == 'wp-admin-bar-get-shortlink') break;
          t = t.parentNode;
        }
        ...
        if (-1 == t.className.indexOf('selected')) t.className += ' selected';

        for (i = 0, l = t.childNodes.length; i < l; i++) {
          node = t.childNodes[i];
          if (
            node.className &&
            -1 != node.className.indexOf('shortlink-input')
          ) {
            node.focus();
            node.select();
            node.onblur = function() {
              t.className = t.className
                ? t.className.replace(rselected, '')
                : '';
            };
            break;
          }
    ...
    addEvent(w, 'load', function() {
      ...
      if (d.body && aB) {
        ...
        if (aB.className) aB.className = aB.className.replace(/nojs/, '');
        ...
        addEvent(
          document.getElementById('wp-admin-bar-logout'),
          'click',
          function() {
            if ('sessionStorage' in window) {
              try {
