...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = (function(
  ...
  if (api.selectiveRefresh) {
    ...
    api.selectiveRefresh.partialConstructor.nav_menu_instance =
      self.NavMenuInstancePartial;
    ...
    self.handleUnplacedNavMenuInstances = function(navMenuInstanceArgs) {
      ...
      unplacedNavMenuInstances = _.filter(
        _.values(self.data.navMenuInstanceArgs),
        function(args) {
          return !api.selectiveRefresh.partial.has(
            'nav_menu_instance[' + args.args_hmac + ']',
          );
        },
      );
    ...
    self.onChangeNavMenuLocationsSetting = function() {
      ...
      hasNavMenuInstance = !!_.findWhere(
        _.values(self.data.navMenuInstanceArgs),
        {
          theme_location: setting._navMenuThemeLocation,
        },
    ...
    $(document).on('click', selector, function(e) {
      ...
      if (navMenuItemParts) {
        ...
        api.preview.send(
          ...
          parseInt(navMenuItemParts[1], 10),
        );
      }
    });
  };
