  ...
  wpList = {
    ...
    pre: function(list, settings, action) {
      ...
      if ($.isFunction(settings.confirm)) {
        ...
        confirmed = settings.confirm.call(
          this,
          list,
          settings,
          action,
    ...
    ajaxAdd: function(element, settings) {
      ...
      settings.element =
        data[2] || $element.prop('id') || settings.element || null;
      ...
      settings.success = function(response) {
        parsedResponse = wpAjax.parseAjaxResponse(
          response,
          settings.response,
          settings.element,
        );
    ...
    ajaxDel: function(element, settings) {
      ...
      if ('none' !== settings.delColor) {
        $eventTarget
          .css('backgroundColor', settings.delColor)
    ...
    ajaxDim: function(element, settings) {
      ...
      $eventTarget = $('#' + settings.element);
      isClass = $eventTarget
        .toggleClass(settings.dimClass)
        .is('.' + settings.dimClass);
      ...
      settings.success = function(response) {
        ...
        if (
          'undefined' !==
          typeof parsedResponse.responses[0].supplemental.comment_link
        ) {
          ...
          if ('' !== parsedResponse.responses[0].supplemental.comment_link) {
            $submittedOn.html(
              $('<a></a>')
                ...
                .prop(
                  'href',
                  parsedResponse.responses[0].supplemental.comment_link,
                ),
            );
    ...
    add: function(element, settings) {
      ...
      if (
        settings.id &&
        (settings.id !== settings.oldId || !old || !old.length)
      ) {
      ...
      if (old && old.length) {
        ...
        'comment' !== settings.what ||
        0 === $('#' + settings.element).length
      ) {
      ...
      if (settings.alt) {
        $element.toggleClass(
          settings.alt,
          ($list.children(':visible').index($element[0]) + settings.altOffset) %
            2,
        );
      }
