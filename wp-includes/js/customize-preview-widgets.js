  ...
  self.WidgetPartial = api.selectiveRefresh.Partial.extend(
    /** @lends wp.customize.widgetsPreview.WidgetPartial.prototype */ {
      ...
      initialize: function(id, options) {
        ...
        api.selectiveRefresh.Partial.prototype.initialize.call(
          partial,
          id,
          options,
        );
      ...
      renderContent: function(placement) {
        var partial = this;
        if (
          api.selectiveRefresh.Partial.prototype.renderContent.call(
            partial,
            placement,
          )
        ) {
    ...
    /** @lends wp.customize.widgetsPreview.SidebarPartial.prototype */ {
      ...
      initialize: function(id, options) {
        ...
        if (partial.params.settings.length > 1) {
          throw new Error(
            'Expected SidebarPartial to only have one associated setting',
          );
        }
      },
      ...
      ready: function() {
        ...
        _.each(sidebarPartial.settings(), function(settingId) {
          api(settingId).bind(
            _.bind(sidebarPartial.handleSettingChange, sidebarPartial),
          );
        });
      ...
      placements: function() {
        var partial = this;
        return _.map(partial.findDynamicSidebarBoundaryNodes(), function(
          boundaryNodes,
        ) {
      ...
      reflowWidgets: function() {
        ...
        _.each(widgetIds, function(widgetId) {
          var widgetPartial = api.selectiveRefresh.partial(
            'widget[' + widgetId + ']',
          );
          if (widgetPartial) {
        ...
        _.each(sidebarPlacements, function(sidebarPlacement) {
          ...
          if (needsSort) {
            _.each(sidebarWidgets, function(sidebarWidget) {
              ...
              api.selectiveRefresh.trigger(
                'partial-content-moved',
                sidebarWidget.placement,
              );
            });
      ...
      ensureWidgetPlacementContainers: function(widgetId) {
        ...
        _.each(sidebarPartial.placements(), function(sidebarPlacement) {
          ...
          foundWidgetPlacement = _.find(widgetPartial.placements(), function(
            widgetPlacement,
          ) {
            return (
              widgetPlacement.context.sidebar_instance_number ===
      ...
      handleSettingChange: function(newWidgetIds, oldWidgetIds) {
            ...
            _.each(widgetPartial.placements(), function(placement) {
              var isRemoved =
                ...
                (placement.context.sidebar_args &&
                  placement.context.sidebar_args.id ===
                    sidebarPartial.sidebarId);
              if (isRemoved) {
        ...
        _.each(widgetsAdded, function(addedWidgetId) {
          var widgetPartial = sidebarPartial.ensureWidgetPlacementContainers(
            addedWidgetId,
          );
