...
(function($, _) {
  ...
  wp.media.string = {
    ...
    props: function(props, attachment) {
      ...
      link =
        props.link || defaultProps.link || getUserSetting('urlbutton', 'file');
    ...
    _audioVideo: function(type, props, attachment) {
      ...
      props = wp.media.string.props(props, attachment);
      if (props.link !== 'embed') return wp.media.string.link(props);
      ...
      if ('video' === type) {
        if (
          attachment.image &&
          -1 === attachment.image.src.indexOf(attachment.icon)
        ) {
    ...
    return _.extend(
      /** @lends wp.media.collection.prototype */ {
        ...
        edit: function(content) {
          ...
          if (
            _.isUndefined(shortcode.get('id')) &&
            !_.isUndefined(defaultPostId)
          ) {
          ...
          if (
            shortcode.attrs.named.type &&
            'video' === shortcode.attrs.named.type
          ) {
  ...
  wp.media.gallery = new wp.media.collection({
    ...
    setDefaults: function(attrs) {
      ...
      _.each(this.defaults, function(value, key) {
        ...
        if (
          value === attrs[key] &&
          (!changed || value === wp.media._galleryDefaults[key])
        ) {
          delete attrs[key];
        }
      });
  ...
  wp.media.editor = {
    ...
    open: function(id, options) {
      ...
      if (
        !workflow ||
        (workflow.options && options.state !== workflow.options.state)
      ) {
