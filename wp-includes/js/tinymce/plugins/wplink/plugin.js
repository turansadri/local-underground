(function(tinymce) {
  ...
  tinymce.PluginManager.add('wplink', function(editor) {
    ...
    function removePlaceholderStrings(content, dataAttr) {
      return content.replace(/(<a [^>]+>)([\s\S]*?)<\/a>/g, function(
        all,
        tag,
        text,
      ) {
    ...
    function checkLink(node) {
      ...
      if (
        /^http/i.test(href) &&
        (!urlRegex1.test(href) || !urlRegex2.test(href))
      ) {
        hasLinkError = true;
    ...
    editor.on('preinit', function() {
      if (editor.wp && editor.wp._createToolbar) {
        ...
        editToolbar.on('show', function() {
          if (
            typeof window.wpLink === 'undefined' ||
            !window.wpLink.modalOpen
          ) {
            window.setTimeout(function() {
              var element = editToolbar.$el.find(
                  'input.ui-autocomplete-input',
                )[0],
                selection =
                  linkNode && (linkNode.textContent || linkNode.innerText);
    ...
    editor.on('pastepreprocess', function(event) {
      ...
      if (
        ...
        !regExp.test(editor.selection.getContent())
      ) {
            ...
            .autocomplete({
              ...
              focus: function(event, ui) {
                $input.attr(
                  ...
                  'mce-wp-autocomplete-' + ui.item.ID,
                );
                /*
              ...
              select: function(event, ui) {
                ...
                if (
                  9 === event.keyCode &&
                  typeof window.wpLinkL10n !== 'undefined'
                ) {
              ...
              messages: {
                ...
                results: function(number) {
                  if (typeof window.uiAutocompleteL10n !== 'undefined') {
                    if (number > 1) {
                      return window.uiAutocompleteL10n.manyResults.replace(
                        '%d',
                        number,
                      );
                    }
      ...
      if (linkNode) {
        ...
        if (href === '_wp_link_placeholder' || edit) {
          ...
          if ($linkNode.attr('data-wplink-url-error') === 'true') {
            ...
            toolbar.$el
              .find('.wp-link-preview a')
              .removeClass('wplink-url-error');
            hasLinkError = false;
          }
        }
