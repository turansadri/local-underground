...
(function(tinymce) {
  ...
  tinymce.PluginManager.add('wordpress', function(editor) {
    var wpAdvButton,
      ...
      wp = window.wp,
      hasWpautop =
        wp && wp.editor && wp.editor.autop && editor.getParam('wpautop', true),
      wpTooltips = false;
    ...
    editor.on('PostRender', function() {
      if (
        editor.getParam('wordpress_adv_hidden', true) &&
        getUserSetting('hidetb', '0') === '0'
      ) {
        toggleToolbars('hide');
      } else {
        tinymce
          .$('.block-library-classic__toolbar')
          .addClass('has-advanced-toolbar');
      }
    });
    ...
    editor.on('BeforeSetContent', function(event) {
      ...
      if (event.content) {
        if (event.content.indexOf('<!--more') !== -1) {
          ...
          event.content = event.content.replace(/<!--more(.*?)-->/g, function(
            match,
            moretext,
          ) {
            return (
        ...
        if (
          event.content.indexOf('<script') !== -1 ||
          event.content.indexOf('<style') !== -1
        ) {
          event.content = event.content.replace(
    ...
    editor.addCommand('WP_More', function(tag) {
      ...
      if (
        node === rootNode ||
        (node.nodeName === 'P' && node.parentNode === rootNode)
      ) {
    ...
    editor.addCommand('WP_Help', function() {
      ...
      if (
        editor.plugins.wptextpattern &&
        (!tinymce.Env.ie || tinymce.Env.ie > 8)
      ) {
    ...
    editor.on('preInit', function() {
      ...
      if (tinymce.Env.mac) {
        tinymce
          ...
          .attr(
            'title',
            __('Rich Text Area. Press Control-Option-H for help.'),
          );
      }
    });
    ...
    editor.on('PastePlainTextToggle', function(event) {
      ...
      if (event.state === true) {
        var times =
          parseInt(
            window.getUserSetting('editor_plain_text_paste_warning'),
            10,
          ) || 0;
        ...
        function create(buttons, bottom) {
          ...
          function reposition() {
            ...
            var scrollX =
              ...
              wpAdminbarBottom = wpAdminbar
                ? wpAdminbar.getBoundingClientRect().bottom
                : 0,
              mceToolbarBottom = mceToolbar
                ? mceToolbar.getBoundingClientRect().bottom
                : 0,
            ...
            if (this.bottom) {
              if (spaceBottom >= spaceNeeded) {
                ...
                top =
                  selection.bottom + iframeRect.top + scrollY - iosOffsetBottom;
              ...
              if (spaceTop >= spaceNeeded) {
                ...
                editorHeight / 2 >
                  selection.bottom + iframeRect.top - blockedTop
              ) {
                className = ' mce-arrow-up';
                top =
                  selection.bottom + iframeRect.top + scrollY - iosOffsetBottom;
            ...
            left =
              selectionMiddle - toolbarWidth / 2 + iframeRect.left + scrollX;

            if (selection.left < 0 || selection.right > iframeRect.width) {
              left =
                iframeRect.left +
                scrollX +
                (iframeRect.width - toolbarWidth) / 2;
            } else if (toolbarWidth >= windowWidth) {
              className += ' mce-arrow-full';
              left = 0;
            } else if (
              (left < 0 && selection.left + toolbarWidth > windowWidth) ||
              (left + toolbarWidth > windowWidth &&
                selection.right - toolbarWidth < 0)
            ) {
              ...
              left + toolbarWidth >
              iframeRect.width + iframeRect.left + scrollX
            ) {
            ...
            toolbar.className =
              toolbar.className.replace(/ ?mce-arrow-[\w]+/g, '') + className;
          ...
          toolbar.on('hide', function() {
            if (isChromeRtl) {
              tinymce
                .$('.mce-widget.mce-tooltip')
                .removeClass('wp-hide-mce-tooltip');
            }
          });
