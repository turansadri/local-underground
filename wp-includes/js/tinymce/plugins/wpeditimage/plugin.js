...
tinymce.PluginManager.add('wpeditimage', function(editor) {
      ...
      editor.addButton('wp_img_' + name, {
        ...
        onPostRender: function() {
          ...
          editor.on('NodeChange', function(event) {
            ...
            node =
              editor.dom.getParent(event.element, '.wp-caption') ||
              event.element;
  ...
  function parseShortcode(content) {
    return content.replace(
      ...
      function(a, b, c) {
        ...
        classes =
          classes && classes[1] ? ' ' + classes[1].replace(/[<>&]+/g, '') : '';
  ...
  function extractImageData(imageNode) {
    ...
    if (imageNode.parentNode && imageNode.parentNode.nodeName === 'A') {
      ...
      metadata.linkTargetBlank =
        dom.getAttrib(link, 'target') === '_blank' ? true : false;
  ...
  function hasTextContent(node) {
    return (
      node && !!(node.textContent || node.innerText).replace(/\ufeff/g, '')
    );
  }
  ...
  function verifyHTML(caption) {
    if (
      !caption ||
      (caption.indexOf('<') === -1 && caption.indexOf('>') === -1)
    ) {
      return caption;
    }
    ...
    return serializer.serialize(
      editor.parser.parse(caption, { forced_root_block: false }),
    );
  }
  ...
  function updateImage($imageNode, imageData) {
    ...
    if (imageData.caption) {
      ...
      id = imageData.attachment_id
        ? 'attachment_' + imageData.attachment_id
        : null;
    ...
    if (tinymce.Env.ie && tinymce.Env.ie > 10) {
      ...
      dom.bind(editor.getBody(), 'mscontrolselect', function(event) {
        if (
          ...
          dom.hasClass(event.target, 'wp-caption')
        ) {
  ...
  editor.on('pastePostProcess', function(event) {
    ...
    if (editor.dom.getParent(editor.selection.getNode(), 'dd.wp-caption-dd')) {
      ...
      editor
        .$(
          'img, audio, video, object, embed, iframe, script, style',
          event.node,
        )
        .remove();
  ...
  editor.on('BeforeExecCommand', function(event) {
    ...
    if (cmd === 'mceInsertContent' || cmd === 'Indent' || cmd === 'Outdent') {
      ...
      node.className = trim(
        node.className.replace(/ ?align(left|center|right|none)/g, '') +
          replacement,
      );
    ...
    editor.on('dragstart', function() {
      ...
      if (node.nodeName === 'IMG') {
        ...
        if (
          ...
          !hasTextContent(node.parentNode)
        ) {
