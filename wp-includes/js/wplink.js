...
(function($, wpLinkL10n, wp) {
  ...
  window.wpLink = {
    ...
    init: function() {
      ...
      inputs.queryNoticeTextHint = inputs.queryNotice.find(
        '.query-notice-hint',
      );
    ...
    hasSelectedText: function(linkNode) {
      ...
      if (linkNode.length) {
        ...
        for (i = nodes.length - 1; i >= 0; i--) {
          ...
          if (
            node.nodeType != 3 &&
            !window.tinymce.dom.BookmarkManager.isBookmarkNode(node)
          ) {
            return false;
          }
        }
    ...
    mceRefresh: function(searchStr, text) {
      ...
      if (linkNode.length) {
        ...
        if (
          searchStr &&
          (urlRegexp.test(searchStr) || emailRegexp.test(searchStr))
        ) {
          href = searchStr;
        }
        ...
        linkText =
          editor.selection.getContent({ format: 'text' }) || text || '';
        this.setDefaultValues(linkText);
      }
    ...
    process: function(results, params) {
      ...
      if (!results) {
        ...
        $.each(results, function() {
          ...
          list +=
            '</span><span class="item-info">' + this.info + '</span></li>';
          alt = !alt;
        });
      }
